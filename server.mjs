...
const AFTER_STAGE_HINTS = {
  ...
  NW4:  "This user is Norwood 4: significant frontal hairline retreat and pronounced crown thinning. Restore density in both the frontal zone (to a credible age-appropriate hairline, not a teenager's) and the crown/vertex. The result should look 'clearly improved, not fully restored' — the crown must still show visible thinning and exposed scalp even in the best-case result; complete crown coverage at NW4 is not realistic.",
...
- potential: realistic percentage improvement achievable with a consistent 6-12 month treatment protocol (OTC or Rx already in the user's "Current routine"). Score what is ACHIEVABLE — not the current state. If "Current routine" contains any Rx antiandrogen — finasteride, dutasteride, spironolactone, bicalutamide, flutamide, cyproterone, or androcur — these drugs substantially improve outcomes at every stage by blocking androgen-driven follicle miniaturization; treat their presence as meaningful upward pressure on potential. Use these stage×age guidelines:
  ...
  • diffuse/female pattern: 55-78 (often nutritional or hormonal — responds well if cause found; for n/a (female) sub-ranges: Ludwig I mild central-part widening→65-78, Ludwig II moderate diffuse thinning→55-65, Ludwig III severe thinning→45-58; infer Ludwig severity from the crown/density scores already assigned above — crown 72-88 → Ludwig I, 48-72 → Ludwig II, 22-48 → Ludwig III)
  ...
  Age unspecified: use the median bracket for each stage (NW3 → apply the 35-54 range; NW3v → apply the 35-54 range; NW4 → apply the over-40 range; NW5+ → any-age range); skip all age-specific upward or downward adjustments; do NOT guess or infer an age penalty when age is absent.
...
Use a balanced visual baseline: score what is actually visible in the photo and user context. Do not artificially lower scores for healthy-looking or stable-looking areas, and do not push users into a low range just for motivation. Typical mild early thinning: 62-78 overall; clearly healthy cases: 78-90; significant recession or density loss: 35-62; 91+ is rare. If photoQuality is 'poor', use a conservative uncertainty range around 62-76 and reflect uncertainty in the verdict. Never refuse — produce a best-effort estimate. Insights array MUST contain exactly 3 entries.`;
      ...
      const scanPromise = (async () => {
        ...
        const _hasMinoxidil  = _routineItems.some((r) => r.includes('minoxidil') || r.includes('rogaine') || r.includes('regaine') || r.includes('minox') || r.includes('kirkland') || r.includes('tugain') || r.includes('mintop') || r.includes('loniten') || r.includes('nanoxidil'));
        const _hasDHTShampoo = _routineItems.some((r) => r.includes('dht') || r.includes('ketoconazole') || r.includes('nizoral') || r.includes('keto shampoo') || r.includes('caffeine shampoo') || r.includes('regenepure') || r.includes('alpecin') || r.includes('plantur') || r.includes('foligain') || r.includes('lipogaine') || r.includes('revita') || r.includes('pura d') || r.includes('shapiro md') || r.includes('rosemary oil') || r.includes('mielle') || r.includes('maple holistics') || r.includes('nioxin') || r.includes('keranique') || r.includes('ultrax') || r.includes('phytocyane') || r.includes('bioxsine') || r.includes('watermans'));
        const _hasMassage    = _routineItems.some((r) => r.includes('massage') || r.includes('dermaroller') || r.includes('derma roller') || r.includes('dermapen') || r.includes('microneedl') || r.includes('micro-needl') || r.includes('dr pen') || r.includes('drpen') || r.includes('dr.pen') || r.includes('zgts') || r.includes('lllt') || r.includes('laser cap') || r.includes('laser comb') || r.includes('capillus') || r.includes('hairmax') || r.includes('irestore') || r.includes('igrow') || r.includes('theradome') || r.includes('kiierr') || r.includes('illumiflow'));
        const _hasLLLT       = _routineItems.some((r) => r.includes('lllt') || r.includes('laser cap') || r.includes('laser comb') || r.includes('capillus') || r.includes('hairmax') || r.includes('irestore') || r.includes('igrow') || r.includes('theradome') || r.includes('kiierr') || r.includes('illumiflow'));
        const _hasMicroneedling = _routineItems.some((r) => r.includes('microneedl') || r.includes('micro-needl') || r.includes('dermaroller') || r.includes('derma roller') || r.includes('dermapen') || r.includes('dr pen') || r.includes('drpen') || r.includes('dr.pen') || r.includes('zgts'));
        const _hasSupplements= _routineItems.some((r) => r.includes('supplement') || r.includes('biotin') || r.includes('vitamin') || r.includes('zinc') || r.includes('saw palmetto') || r.includes('nutrafol') || r.includes('viviscal') || (r.includes('iron') && !r.includes('flat iron') && !r.includes('curling iron') && !r.includes('steam iron') && !r.includes('hair iron') && !r.includes('flat-iron') && !r.includes('curling-iron')) || r.includes('pumpkin seed') || r.includes('folexin') || r.includes('hairfinity') || r.includes('perfectil') || r.includes('hairburst') || r.includes('collagen') || (r.includes('keratin') && !r.includes('keratin treatment') && !r.includes('keratin therapy') && !r.includes('keratin complex') && !r.includes('keratin smoothing') && !r.includes('keratin straighten') && !r.includes('keratin blowout')) || r.includes('marine collagen') || r.includes('hair formula') || r.includes('omega') || r.includes('fish oil') || r.includes('folic acid') || r.includes('folate') || r.includes('silica') || r.includes('niacin') || r.includes('evening primrose') || r.includes('selenium') || r.includes('magnesium') || r.includes('copper') || r.includes('lysine') || r.includes('msm') || r.includes('ashwagandha') || r.includes('nettle') || r.includes('beta-sitosterol') || r.includes('hair gum') || r.includes('multivitamin') || r.includes('nourkrin') || r.includes('priorin') || r.includes('hair vitalics') || r.includes('pantogar') || r.includes('bhringraj') || r.includes('sugarbear') || r.includes('vegamour') || r.includes('hair la vie') || r.includes('foligrowth') || r.includes('pantovigar') || r.includes('philip kingsley') || r.includes('tricho complex') || r.includes('florisene') || r.includes('lambdapil') || r.includes('hum nutrition'));
        const _hasFinasteride = _routineItems.some((r) => r.includes('finasteride') || r.includes('propecia') || r.includes('dutasteride') || r.includes('avodart') || r.includes('proscar') || r.includes('finpecia') || r.includes('finalo') || r.includes('finast') || r.includes('fincar') || r.includes('finax') || r.includes('aindeem') || r.includes('spironolactone') || r.includes('spiro') || r.includes('aldactone') || r.includes('bicalutamide') || r.includes('flutamide') || r.includes('cyproterone') || r.includes('androcur'));
      ...
      const protocolStatusLine = (() => {
        ...
        if (!pc && ctx.routine.length > 0) {
          ...
          pc = {
            ...
            dhtShampoo:  r.some((s) => s.includes('dht') || s.includes('ketoconazole') || s.includes('nizoral') || s.includes('keto shampoo') || s.includes('caffeine shampoo') || s.includes('regenepure') || s.includes('alpecin') || s.includes('plantur') || s.includes('foligain') || s.includes('lipogaine') || s.includes('revita') || s.includes('pura d') || s.includes('shapiro md') || s.includes('rosemary oil') || s.includes('mielle') || s.includes('maple holistics') || s.includes('nioxin') || s.includes('keranique') || s.includes('ultrax') || s.includes('phytocyane') || s.includes('bioxsine') || s.includes('watermans')),
            mechanical:    r.some((s) => s.includes('massage') || s.includes('dermaroller') || s.includes('derma roller') || s.includes('dermapen') || s.includes('microneedl') || s.includes('micro-needl') || s.includes('dr pen') || s.includes('drpen') || s.includes('dr.pen') || s.includes('zgts') || s.includes('lllt') || s.includes('laser cap') || s.includes('laser comb') || s.includes('capillus') || s.includes('hairmax') || s.includes('irestore') || s.includes('igrow') || s.includes('theradome') || s.includes('kiierr') || s.includes('illumiflow')),
            microneedling: r.some((s) => s.includes('microneedl') || s.includes('micro-needl') || s.includes('dermaroller') || s.includes('derma roller') || s.includes('dermapen') || s.includes('dr pen') || s.includes('drpen') || s.includes('dr.pen') || s.includes('zgts')),
            lllt:          r.some((s) => s.includes('lllt') || s.includes('laser cap') || s.includes('laser comb') || s.includes('capillus') || s.includes('hairmax') || s.includes('irestore') || s.includes('igrow') || s.includes('theradome') || s.includes('kiierr') || s.includes('illumiflow')),
            supplements:   r.some((s) => s.includes('supplement') || s.includes('biotin') || s.includes('vitamin') || s.includes('zinc') || s.includes('saw palmetto') || s.includes('nutrafol') || s.includes('viviscal') || (s.includes('iron') && !s.includes('flat iron') && !s.includes('curling iron') && !s.includes('steam iron') && !s.includes('hair iron') && !s.includes('flat-iron') && !s.includes('curling-iron')) || s.includes('pumpkin seed') || s.includes('folexin') || s.includes('hairfinity') || s.includes('perfectil') || s.includes('hairburst') || s.includes('collagen') || (s.includes('keratin') && !s.includes('keratin treatment') && !s.includes('keratin therapy') && !s.includes('keratin complex') && !s.includes('keratin smoothing') && !s.includes('keratin straighten') && !s.includes('keratin blowout')) || s.includes('marine collagen') || s.includes('hair formula') || s.includes('omega') || s.includes('fish oil') || s.includes('folic acid') || s.includes('folate') || s.includes('silica') || s.includes('niacin') || s.includes('evening primrose') || s.includes('selenium') || s.includes('magnesium') || s.includes('copper') || s.includes('lysine') || s.includes('msm') || s.includes('ashwagandha') || s.includes('nettle') || s.includes('beta-sitosterol') || s.includes('hair gum') || s.includes('multivitamin') || s.includes('nourkrin') || s.includes('priorin') || s.includes('hair vitalics') || s.includes('pantogar') || s.includes('bhringraj') || s.includes('sugarbear') || s.includes('vegamour') || s.includes('hair la vie') || s.includes('foligrowth') || s.includes('pantovigar') || s.includes('philip kingsley') || s.includes('tricho complex') || s.includes('florisene') || s.includes('lambdapil') || s.includes('hum nutrition')),
          };
        }
        if (!pc) return '';
        const active = [];
