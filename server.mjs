...
const METRICS = {
  scan:             { requests: 0, errors: 0, cacheHits: 0, slowRequests: 0, retries: 0, promptTokens: 0, completionTokens: 0, truncations: 0, lastError: null, lastSuccess: null, lastRetry: null },
...
CRITICAL wet / product-laden hair rule: if the input photo shows wet hair (post-shower, post-swim — dark shine, slick appearance, strand clumping with visible scalp showing between wet rope-like groups of hair), preserve that wet appearance in the output — do NOT treat the scalp gaps between wet clumps as thinning zones to fill. The correct approach: assess actual hair loss only from the persistent bald or thinning zones where the scalp is bare regardless of moisture (receding hairline, crown bald patch, etc.) — those are the real thinning areas to improve. Do not add dry or fluffy hair on top of a wet scalp; any density improvement must maintain the same wet texture, shine, and clumped appearance as the input. The same rule applies to hair heavily coated with gel, pomade, wax, or mousse — strand clumping from product load is not a thinning zone to restore.
...
const PROGRESSION_STAGE_HINTS = {
  ...
  NW6:  "This user is Norwood 6 (frontal and crown merged; only a lateral fringe remains). Direct any visible improvement to the lateral fringe margins only — slightly denser individual hairs at the perimeter of the sides and nape fringe, proportional to the treatment month. The large merged frontal-crown bald top must remain clearly bare and essentially unchanged at every treatment month — OTC treatment at NW6 maintains the fringe, not the scalp top. Do NOT add any density or darkening to the merged bald area across the top.",
  NW7:  "This user is Norwood 7 (near-total top loss; horseshoe fringe only). Direct any visible improvement to the very edge of the horseshoe fringe only — at most marginally thicker hairs at the fringe boundary, barely perceptible. The near-total bald scalp top must remain essentially bare and unchanged at every treatment month — OTC treatment at NW7 can only maintain the remaining fringe. Do NOT add any density to the bald scalp top.",
...
const buildProgressionPrompt = (month, stage, thinningPattern) => {
  ...
  } else if (month === 12 && stage === 'NW6') {
    qualifier = 'Stage-specific constraint (CALIBRATES the "STRONG / Full natural-looking density" language above — at NW6, OTC treatment at 12 months can at best produce marginal thickening of the remaining lateral fringe hairs along the sides and nape. The large merged frontal-crown bald top must remain clearly bare and essentially unchanged — do NOT add density to the bald top area. Show only slightly denser fringe coverage at the perimeter; a viewer should immediately recognize the same extensive merged bald top, with at best marginally thicker fringe hair at the sides. The overall impression must be "substantially bald scalp top with slightly better fringe maintenance", never "meaningfully improved scalp coverage"):';
  } else if (month === 12 && stage === 'NW7') {
    qualifier = 'Stage-specific constraint (CALIBRATES the "STRONG / Full natural-looking density" language above — at NW7, OTC treatment at 12 months can at most produce marginally thicker hairs at the very edge of the horseshoe fringe. The near-total bald scalp top must remain essentially bare and unchanged — do NOT add any density to the top. The improvement must look nearly imperceptible; a viewer should still see essentially the same near-total loss pattern with only barely-perceptible marginal fringe preservation at the very periphery. The overall impression must be "near-total hair loss, very slightly better fringe maintenance", not improved scalp coverage):';
...
- insights: exactly 3 items, each with a 5-word title (≤5 words), a 20-28 word actionable body that is specific to THIS user's visible loss pattern, scores, stage, or profile — name the actual stage or a score, specify a concrete action, and give a reason tied to their situation. CRITICAL routine rule: check "Current routine" in the user context above — if a treatment is already listed (e.g. minoxidil, finasteride, dutasteride, spironolactone, bicalutamide, flutamide, cyproterone, androcur, DHT-blocking shampoo, supplements), do NOT suggest starting it. Instead, suggest how to optimize that treatment (e.g. proper application coverage, timing, frequency) or recommend a different complementary layer. Never repeat a recommendation for something the user already does. Avoid generic advice. CRITICAL diversity rule: every insight MUST target a DIFFERENT metric — never assign the same metric value to two insights; pick the 3 most clinically relevant distinct metrics from: Hairline, Density, Crown, Health, Potential. The metric must match: Hairline→temple/frontal recession, Density→mid-scalp thinning, Crown→vertex/crown thinning, Health→scalp condition or miniaturization, Potential→treatment response or growth timeline. NW1 calibration: when stage is NW1 (fully intact), there is no visible loss to describe — frame all three insights preventively. Hairline insight: monitoring cadence to catch early M-shape (e.g. monthly front-facing photo from the same angle); Density insight: scalp-health habit that protects mid-scalp follicle caliber now (e.g. DHT-blocking shampoo contact time, scalp massage frequency); Crown insight OR Health insight: nutritional or topical layer that extends the fully-intact window. Reframe every metric as "protect what's there" not "restore what's lost". TE/stress calibration for insights: when Stress ≥ 7/10 or Sleep ≤ 5h in the user context AND stage is 'diffuse' or 'n/a (female)' (uniform thinning without M-shape recession), dedicate at least one insight — preferably Health metric — to the TE root cause: frame it as "Stress ≥ 7 with diffuse thinning suggests TE — typically reverses within 3-6 months once the stressor is managed; prioritize sleep and stress reduction alongside topicals." or "Sleep ≤ 5h is a recognized TE trigger — restoring 7-8h sleep is the highest-leverage action alongside topicals for diffuse loss." Body must be 20-28 words. Do NOT apply this to NW1-NW7 AGA stages with clear temple or vertex recession. Minoxidil-onset TE calibration for insights: when 'Current routine' includes minoxidil AND onset is ≤6 months AND diffuse shedding is visible (not focal temple recession or crown patch), at least one insight — preferably the Health metric — should acknowledge the shedding as a likely normal treatment response and reassure the user. Example body (20-28 words): 'Increased shedding in the first 1–3 months on minoxidil is expected treatment-induced TE — follicles are resetting; stay consistent and reassess results at month 4–6.' Do NOT apply when loss is clearly focal. Finasteride-onset TE calibration for insights: when 'Current routine' includes finasteride (or any finasteride-containing product — Propecia, Proscar, Finpecia, Finalo, Fincar, Finax, Finast, Aindeem, or a topical combo like Morr-F, Tugain-F, or Finalo-F) AND onset is ≤6 months AND shedding appears diffuse rather than focal, at least one insight — preferably the Health metric — should acknowledge the shedding as a likely normal treatment response and reassure the user. Example body (20-28 words): 'Increased shedding in the first 1–4 months on finasteride is a recognized treatment-induced TE response — follicles are resetting; stay consistent and reassess at month 6.' Do NOT apply when loss is clearly focal. Dutasteride-onset TE calibration for insights: when 'Current routine' includes dutasteride (Avodart, Dutas, Duprost, Duodart, or any dutasteride-containing product) AND onset is ≤6 months AND shedding appears diffuse rather than focal, at least one insight — preferably the Health metric — should acknowledge the shedding as a likely normal treatment response and reassure the user. Dutasteride blocks both type 1 and type 2 5-alpha reductase (stronger than finasteride), so the initial TE response can be slightly more pronounced and may persist until month 5–7. Example body (20-28 words): 'Increased shedding in the first 1–4 months on dutasteride is normal treatment-induced TE — deep DHT suppression resets follicles; stay consistent and reassess at month 6.' Do NOT apply when loss is clearly focal. Spironolactone-onset TE calibration for insights: when 'Current routine' includes spironolactone (Aldactone, or any spironolactone-containing product) AND onset is ≤6 months AND shedding appears diffuse rather than focal, at least one insight — preferably the Health metric — should acknowledge the shedding as a likely normal treatment response and reassure the user. Spironolactone is a mineralocorticoid/androgen receptor blocker prescribed primarily for female-pattern hair loss; hormonal adjustment in the first 1–4 months can temporarily increase diffuse shedding before stabilizing. This TE response is self-limiting (typically resolves by month 3–5) and does not indicate treatment failure. Example body (20-28 words): 'Increased shedding in the first 1–4 months on spironolactone is a recognized hormonal adjustment response — follicles are adapting; stay consistent and reassess at month 6.' Do NOT apply when loss is clearly focal (defined temple recession or crown bald patch). Do NOT apply when stage is NW1-NW7 AGA with clear androgenic recession — apply only for diffuse or n/a (female) pattern. Bicalutamide/flutamide/cyproterone-onset TE calibration for insights: when 'Current routine' includes bicalutamide (Casodex or generic), flutamide (Eulexin or generic), or cyproterone acetate (Androcur or generic) AND onset is ≤6 months AND shedding appears diffuse rather than focal, at least one insight — preferably the Health metric — should acknowledge the shedding as a likely normal treatment response and reassure the user. These drugs are androgen receptor blockers prescribed primarily for female-pattern hair loss or as adjuncts in trans-feminine hormone therapy; blocking androgen receptors modulates the hormonal signaling that regulates the hair follicle cycle, and some users experience a transient diffuse shedding adjustment in the first 1–4 months before the hair cycle restabilizes. This TE response is self-limiting (typically resolves by month 2–4 for flutamide and cyproterone, and by month 3–5 for bicalutamide) and does not indicate treatment failure. Example body (20-28 words): 'Increased shedding in the first 1–4 months on [drug name] is a recognized hormonal adjustment response — follicles are adapting; stay consistent and reassess at month 6.' Do NOT apply when loss is clearly focal. Do NOT apply when stage is NW1-NW7 AGA with clear androgenic recession — apply only for diffuse or n/a (female) pattern. Seasonal shedding TE calibration for insights: increased diffuse shedding in late summer and early autumn is a recognized seasonal photoperiod response (August–November in the Northern Hemisphere, February–May in the Southern Hemisphere), peaking 90–120 days after the summer solstice. When: (1) Today's date in the user context falls within a seasonal risk month, AND (2) Concerns include shedding or increased hair loss without a clear AGA-progression signal (no new focal bald patches or noticeably deepened temple recession), AND (3) 'Current routine' has been stable for 6+ months with no new medication starts — at least one insight (Health metric, 20-28 words) should acknowledge seasonal TE as a plausible contributing factor. Example body: 'Increased shedding in late summer is often seasonal photoperiod TE — self-limiting within 2–3 months; stay consistent with your routine and scan again in 6–8 weeks.' Do NOT apply this when treatment-onset TE context exists (new minoxidil, finasteride, or dutasteride start within the past 6 months) — treatment-onset TE takes priority as a more specific explanation. Do NOT apply when clear AGA stage progression is visible (focal temple deepening or enlarging crown patch). Postpartum TE calibration for insights: postpartum telogen effluvium occurs in 40–50% of new mothers within 1–5 months after delivery, caused by the sudden post-partum estrogen drop pushing a large cohort of follicles simultaneously into telogen. It is self-limiting and typically fully resolves within 6–12 months — the follicles themselves are completely intact and healthy. When: (1) 'Onset' or 'Concerns' include any postpartum or post-pregnancy indicator — "postpartum", "post-partum", "after baby", "after pregnancy", "after delivery", "after giving birth", "after childbirth", "months after baby", "post-pregnancy", "postnatal", "postpartum shedding", or similar, AND (2) shedding appears diffuse without a clear AGA progression signal (no new focal bald patches or deepened temple recession), AND (3) stage is 'diffuse' or 'n/a (female)' — at least one insight (Health metric, 20-28 words) should acknowledge postpartum TE as the most likely cause and reassure the user. Example body: 'Postpartum shedding typically peaks 3–5 months after delivery and fully resolves within 6–12 months — follicles are healthy; maintain your current routine and rescan in 6 weeks.' Do NOT apply when treatment-onset TE context is more specific (new medication start within the past 6 months) — that takes priority. Do NOT apply when clear AGA stage progression is visible (focal temple deepening or enlarging crown patch). Postpartum TE takes priority over seasonal TE if both apply simultaneously. n/a (female) metric calibration for insights: female-pattern hair loss spares the temples — the primary affected zones are the central parting, crown vertex, and occasionally the frontal hairline (Ludwig III only). When stage is 'n/a (female)': (1) prioritize Crown (vertex density) and Density (central parting width) as the two primary insight metrics; (2) if Hairline is assigned, the insight MUST address central-part width or frontal density preservation — NEVER temple recession, which is absent in female-pattern loss; (3) for the Potential insight, frame recovery around hormonal/nutritional root-cause investigation and minoxidil timeline, not finasteride or DHT-blocking language.
...
- NW1 vs NW2: Assign NW1 only when the hairline sits at or above the upper forehead crease with completely symmetric temples and no frontotemporal angle widening. Any visible M-shape deepening, angular notch at either temple, or even subtle asymmetry → NW2 minimum. Natural mature hairline calibration: some individuals have a naturally higher or more angular frontotemporal hairline that is NOT androgenic recession — a stable variant (sometimes called a "mature hairline") that is common in adults of many ethnic backgrounds including East and Southeast Asian descent. The key distinguisher between a natural angular hairline and early NW2 AGA recession is miniaturization: AGA-driven recession produces visible vellus or miniaturized hairs (shorter, finer, lighter) at the temple edge; a natural hairline shows uniformly terminal (full-thickness) hairs at that same edge with no vellus replacement. Additional signal: if the Concerns or timeline fields mention active temple recession, progression, or shedding → NW2. If no progression is reported and the temple edge hairs appear uniformly full-caliber terminal with no visible vellus transition → consider NW1 with a photoNote acknowledging the natural mature hairline shape and recommend monitoring with periodic photos.
- NW2 vs NW3: If temple recession extends PAST the mid-pupil vertical line, assign NW3. If it barely reaches or falls short of midpupil, assign NW2. Hairline-invisible caveat: When the photo is a top-down overhead or crown-focused shot and the frontotemporal hairline is NOT visible in the frame, the mid-pupil vertical-line discriminator cannot be applied directly. In that case: (a) use the user's 'Concerns' field as the primary signal — if Concerns include any of (deep recession, significant temple loss, corners gone, temples significantly receded, receding for 3+ years, receding a lot), prefer NW3; if Concerns suggest only mild or early recession (first noticed recently, just started, hairline barely moved, slight corners), prefer NW2; (b) when Concerns are silent on recession depth, infer from crown/vertex appearance — an intact crown with the hairline reported as mildly changed suggests NW2, while visible vertex thinning alongside any reported temple recession context suggests NW3 or NW3v; (c) if no directional signal is available from either Concerns or crown visibility, default to NW2 over NW3 because NW3 requires clearly deep bilateral recession past mid-pupil, which is a more specific claim that should not be made without evidence from the photo or Concerns. Do NOT apply this caveat when the frontotemporal hairline is partially or fully visible in the frame. Close-up selfie / wide-angle lens distortion caveat: when the photo appears to be a close-range front-facing selfie (face substantially fills the entire frame, ears are partially or fully cut off at the sides, or the head appears unusually wide relative to its height), wide-angle barrel distortion from the front-facing camera may make the temple recession appear deeper or more laterally displaced than it actually is — the distortion pushes the outer hairline corners toward the frame edges, mimicking the appearance of deeper NW3 recession. When these wide-angle selfie geometry signs are present and the recession appears to be right on the NW2/NW3 boundary, err toward NW2 rather than NW3 unless the Concerns field explicitly confirms deep recession. Note the wide-angle distortion in photoNote and recommend a retake from ~50cm distance or with the rear camera for the most accurate hairline reading.
...
Alopecia areata / non-AGA pattern detection — flag in photoNote when visible:
Alopecia areata (AA) presents as one or more sharply demarcated, completely smooth, round or oval bald patches — in contrast to AGA where recession or thinning is gradual and follows predictable hairline-to-crown patterns. Key visual distinctions:
- Shape: AA patches are perfectly round or oval with abrupt, well-defined borders and a completely smooth bald surface. AGA produces a gradual density gradient or a defined recession front, not isolated smooth patches.
- Location: AA patches can appear anywhere on the scalp (occiput, parietal, temporal, vertex) — they do NOT follow the M-shape recession or crown-vertex patterns characteristic of AGA. AGA recession starts specifically at the frontotemporal hairline or crown vertex and spreads predictably outward.
- Surrounding hair: hair immediately adjacent to an AA patch is typically normal caliber with no AGA-type miniaturization gradient at the patch border. AGA-affected areas show a gradual transition from terminal to vellus/miniaturized hairs.
- Scalp surface: the scalp surface inside an AA patch is smooth and normal-colored — no inflammation, scaling, or redness in the classic form. Unlike AGA crown loss, which shows a gradual fade, AA patches have a sharp, punched-out appearance.
When one or more clearly demarcated, smooth circular or oval bald patches are visible that do NOT fit the gradual AGA recession or crown-thinning geometry, flag this in photoNote with a note such as: "One or more isolated round bald patches visible — this pattern is more consistent with alopecia areata (an autoimmune condition) than androgenetic hair loss. Alopecia areata responds to different treatments than AGA; a dermatologist visit is recommended for diagnosis before starting AGA-targeted treatment." Still assign the closest AGA stage for the app's scoring system (use 'diffuse' if no temple recession is present, or the closest NW stage if temple recession also exists alongside the patch), but note the likely non-AGA pattern clearly in photoNote. Do NOT flag routine AGA density loss, a crown thinning patch typical of NW3v/NW4, or uniform diffuse thinning as alopecia areata — only flag when one or more patches are distinctly round/oval with sharp borders inconsistent with AGA progression geometry.

Traction alopecia / mechanical hair loss detection — flag in photoNote when visible:
Traction alopecia (TA) is hair loss caused by chronic mechanical tension from tight hairstyles — box braids, tight ponytails, buns, cornrows, weaves, dreadlocks, or hair extensions. It presents differently from AGA and requires a different treatment approach. Key visual distinctions:
- Location and geometry: TA typically produces a band-like or marginal hairline recession along the frontotemporal line, often bilateral and relatively symmetric, but WITHOUT the classic angular M-shape corners of AGA. The recession line tends to be straighter, following the hairline margin rather than producing deep angular temple notches. Sometimes the recession is specifically periauricular — at the sideburn/temple junction, behind the ears, or at the nape — zones under the highest tension from braids or weaves.
- Crown and vertex spared: TA almost always spares the crown and vertex, which remain fully dense even when the frontal or temporal hairline is visibly receding. A pattern of clear frontal or periauricular hairline recession with a completely intact dense crown is more consistent with TA than AGA (AGA at an equivalent recession depth would typically show at least some crown thinning at NW3v or NW4 in a male, and central-part widening in a female).
- Who it affects: TA is most common in female users — particularly those with Afro-textured, South Asian, or East Asian hair worn in tight hairstyles — but can affect any hair type subjected to chronic pulling tension. Males with very tight cornrows, dreadlocks, or man buns are also susceptible.
- Styling context signals: if the visible hair shows signs of tight braiding, locs, extensions, or a tight updo (e.g. scalp tension visible at the hairline margin, hair pulled uniformly away from the scalp at the edges), this raises TA suspicion alongside a non-AGA pattern.
When frontal or periauricular hairline recession is visible that does NOT follow the classic M-shape AGA geometry AND the crown and vertex appear fully dense AND the pattern (linear marginal recession or periauricular recession) is geometrically inconsistent with AGA progression, flag this in photoNote with a note such as: "Frontotemporal or periauricular hairline recession without crown involvement — this pattern may be consistent with traction alopecia from chronic tight hairstyling rather than androgenetic hair loss. Traction alopecia's first-line treatment is stopping the pulling hairstyle; topical minoxidil can support early regrowth. AGA-targeted DHT treatments are not the primary treatment for traction-related loss. A dermatologist or trichologist can help confirm the cause." Still assign the closest AGA stage for the app's scoring system (NW2 or NW3 if frontal recession is present; 'n/a (female)' for female users). Do NOT flag standard M-shape AGA temple recession (which shows characteristic angular corner deepening) as traction alopecia — only flag when the recession geometry is clearly linear, band-like, or periauricular rather than the angular temple-corner retreat typical of AGA. When both AGA and TA may be co-present, note both possibilities in photoNote.

Frontal Fibrosing Alopecia (FFA) / scarring alopecia detection — flag in photoNote when visible:
Frontal Fibrosing Alopecia (FFA) is a primary lymphocytic scarring (cicatricial) alopecia that progressively destroys hair follicles at the frontotemporal hairline margin. It is one of the most common scarring alopecias and is frequently misidentified as AGA, leading to delayed diagnosis and inadequate treatment. FFA requires anti-inflammatory treatment; standard AGA therapies alone (minoxidil, finasteride) are not adequate. Key visual distinctions from AGA:
- Recession geometry: FFA recession is a fairly uniform BAND across the entire frontotemporal hairline — it is straighter and more evenly distributed across the full width than AGA, which produces distinct angular M-shape corner notching. FFA typically recedes the entire frontal hairline margin back by a relatively uniform amount, not the bilateral angular deepening at the temple corners that defines AGA.
- Crown preserved: unlike AGA (where an equivalent recession depth would typically involve crown thinning at NW3v-NW4), FFA almost always leaves the crown and mid-scalp completely intact and dense even when the frontotemporal margin has receded substantially. This hair-intact-crown + receding-frontal-band pattern is a key clinical signal.
- Peripilar casts: the pathognomonic sign of FFA is a ring of fine whitish scale (peripilar cast) surrounding individual hair follicle openings at the leading edge of the recession. These look like tiny white collars or cuffs around each follicle at the recession margin — distinct from dandruff (which is flaky, diffuse, and not follicle-centered). Peripilar casts may be visible in close-up photos as fine white rings at the hairline margin.
- Lonely hair sign: isolated single hairs ("lonely hairs") surviving at the otherwise bare frontotemporal margin, often with peripilar scale at their base, are a recognized sign of FFA. These isolated terminal hairs stand out against the otherwise smooth, hairless recession margin.
- Pale band: a thin strip of slightly pale or erythematous (reddened) scalp along the advancing recession margin may be visible as a distinct pale or mildly inflamed zone at the hairline edge — different from the gradual density gradient of AGA recession.
- Eyebrow involvement: FFA often causes progressive loss at the outer third (lateral tail) of the eyebrows. If the lateral eyebrows appear sparse, patchy, or absent in the photo while the frontal hairline shows uniform band-like recession, this raises FFA suspicion significantly. Eyebrow loss at the outer corners combined with frontotemporal band recession is a strongly suggestive combination.
- Who it affects: FFA predominantly affects post-menopausal women (over 50) but is increasingly diagnosed in women aged 30–50 and occasionally in men. Any female user presenting with a uniform frontotemporal band recession without M-shape angles and with a fully intact crown warrants FFA consideration, especially if eyebrow involvement is visible.
When a uniform band-like frontotemporal hairline recession is visible that lacks the M-shape angular deepening of AGA AND the crown and mid-scalp are clearly intact AND optional signals (peripilar casts, lonely hairs, eyebrow lateral thinning, pale or erythematous margin) are present, flag this in photoNote with a note such as: "Uniform band-like frontotemporal hairline recession with an intact crown — this pattern may be consistent with Frontal Fibrosing Alopecia (FFA), a primary scarring alopecia rather than androgenetic hair loss. FFA is progressive and requires anti-inflammatory treatment (immunomodulatory drugs, possibly finasteride or dutasteride as adjuncts in some protocols); standard topical minoxidil alone is not adequate. If you notice fine white scale around individual follicles at your hairline margin, thinning at the outer corners of your eyebrows, or a pale strip at the hairline edge, please see a dermatologist or trichologist promptly for evaluation — early diagnosis significantly slows FFA progression." Still assign the closest AGA stage for the app's scoring system (NW2 or NW3 if significant frontal recession is present; 'n/a (female)' for female users — do NOT assign 'diffuse' for FFA since the recession is localized to the frontal margin, not uniform thinning across the top). Do NOT flag standard M-shape angular AGA temple recession as FFA — FFA's recession is a band across the full frontal width without the bilateral corner-notching that defines AGA. Do NOT apply FFA suspicion to diffuse thinning or crown-only patterns — FFA is specifically a frontotemporal margin condition. When both AGA and FFA may be co-present (some patients develop AGA on top of FFA), note both possibilities in photoNote.

Diffuse Unpatterned Alopecia (DUPA) detection — flag in photoNote when visible:
DUPA is a variant of androgenetic alopecia in which DHT sensitivity is not limited to the scalp top — it also affects the occipital and lateral fringe ("horseshoe") zones that normally serve as safe donor areas in standard AGA. This distinction is clinically critical: if a DUPA patient pursues FUE, FUT, or DHI hair transplantation, the transplanted donor hair is itself DHT-sensitive and will continue to miniaturize after transplantation, producing poor long-term outcomes and wasted grafts.
Key visual distinguisher from standard AGA:
- In standard AGA (NW5–NW7): the horseshoe fringe (lateral sides and occipital nape) appears dense, full, and composed of thick terminal hairs — the fringe is the "safe zone" that provides transplant donor stock.
- In DUPA: even the horseshoe fringe appears thin, sparse, or miniaturized — fringe hairs look finer, shorter, or wispier than expected; scalp may be visible through the fringe even where standard AGA would show dense terminal hair. The entire scalp surface, including the sides and nape, looks diffusely affected rather than patterned.
- Photo context where DUPA is assessable: any photo that shows the lateral or occipital fringe — a frontal selfie where the sides are visible, an overhead shot where the fringe perimeter is in frame, or a side-profile photo. DUPA cannot be confirmed from a pure top-down overhead shot that shows only the crown/vertex.
When advanced hair loss is visible on the scalp top (NW5–NW7 equivalent extent) AND the visible lateral or occipital fringe also appears clearly sparse, miniaturized, or has noticeable scalp show-through between fringe hairs (inconsistent with the dense full fringe expected in standard advanced AGA), flag this in photoNote with a note such as: "Advanced scalp-top hair loss with visible thinning of the lateral or occipital fringe — this pattern may be consistent with Diffuse Unpatterned Alopecia (DUPA), a variant where DHT sensitivity affects the entire scalp including the traditional safe donor zone. DUPA patients are generally poor candidates for hair transplantation because donor-zone hair is also miniaturization-prone and may continue to fall after transplantation. A dermatologist or trichologist consultation with dermoscopy of the donor zone is strongly recommended before considering any surgical restoration." Still assign the closest AGA stage for the app's scoring system based on the visible extent of the scalp top loss. Do NOT apply DUPA suspicion based on top/crown loss alone — it requires a visibly compromised fringe zone. Do NOT flag early NW1–NW4 stages as DUPA, since the fringe is typically intact at those stages even in DUPA. Do NOT confuse very short clipper-cut fringe (where scalp shows through due to hair length) with DUPA miniaturization — DUPA fringe hairs appear thin and wispy, not uniformly short like a clipper cut.

Seborrheic dermatitis (SD) / scalp psoriasis detection — flag in photoNote when visible:
Seborrheic dermatitis is a chronic inflammatory scalp condition driven by Malassezia yeast overgrowth on sebaceous-rich skin. It affects 3–5% of the general population and co-occurs with AGA at high rates. SD is clinically important because it exacerbates androgenetic hair loss through peri-follicular inflammation that shortens the anagen growth phase and accelerates follicle miniaturization. First-line treatment (ketoconazole 2% antifungal shampoo, not generic dandruff shampoos) is different from AGA protocol — identifying it early helps the user add the correct scalp-health layer. Scalp psoriasis is a separate but visually related inflammatory condition that also requires specific treatment distinct from AGA therapy.
Key visual signs of seborrheic dermatitis:
- Greasy, yellowish or white flakes adhering to the scalp surface and hair shafts — typically concentrated at the hairline margin, the central scalp parting, behind the ears, or at the nape. Unlike dry environmental dandruff (white, powdery, easily dislodged), SD flakes appear oily or slightly translucent and may clump at the base of hair shafts.
- Erythema (redness or pinkish discoloration) of the scalp, often visible at the hairline margin, around the ears, on the central scalp, or in the eyebrow area — sometimes appearing as a faint reddish border at the frontal hairline edge.
- An oily, shiny scalp texture visible even in photos — sometimes accompanied by a yellowish or salmon-colored sheen or crust at the hairline edge.
- Involvement of the eyebrow area (scaling or redness at the eyebrow inner margin) alongside scalp flaking — co-involvement of face and scalp is a strong SD indicator.
Key visual signs of scalp psoriasis (distinct from SD):
- Thick, silvery-white or whitish-grey plaques with well-defined, raised borders — the plaques are drier and more compact than greasy SD flaking, with a distinct scaly, layered texture.
- Pronounced erythema (bright redness) beneath and around the plaques — the erythema in psoriasis is typically more intense and well-demarcated than SD's diffuse pinkish tint.
- The plaques may extend beyond the hairline onto the forehead (an "oil-slick" border along the hairline is characteristic), behind the ears, or at the nape.
When visible greasy/yellowish flaking or scalp erythema at the hairline, central scalp, or behind the ears is present that appears inconsistent with simple dry environmental scalp or post-shower water, flag this in photoNote with a note such as: "Visible scalp flaking or redness at the hairline or scalp surface — this may be consistent with seborrheic dermatitis (SD), a common inflammatory scalp condition that frequently co-occurs with AGA and can worsen hair loss through peri-follicular inflammation. First-line treatment is ketoconazole 2% shampoo (Nizoral or generic ketoconazole) 2–3× weekly with 3–5 minutes of contact time — ketoconazole has both antifungal and mild anti-androgenic effects on the scalp, providing a dual benefit alongside AGA treatment. A dermatologist can confirm the diagnosis and recommend stronger anti-inflammatory adjuncts if ketoconazole shampoo alone is insufficient." When thick silvery plaques with well-defined borders are visible rather than greasy flaking, adjust the note to mention scalp psoriasis and recommend a dermatologist evaluation. Deduct −5 to −15 from the health score based on visible inflammation severity; mild visible erythema without prominent flaking → −5 to −8; prominent greasy flaking at the hairline → −8 to −12; thick inflamed plaques → −10 to −15. Do NOT flag: normal dry scalp, minor white powdery flaking typical of environmental dryness or product residue (white, non-oily, uniformly scattered small flakes), post-shower water spots, or the whitish scale of FFA peripilar casts (which are follicle-centered tiny rings, not diffuse scalp flaking). Only flag when flaking or erythema is clearly seborrheic in character (greasy, yellowish or oily-white, concentrated at sebaceous zones — hairline, central scalp, behind ears) or psoriatic in character (thick, silvery, well-defined plaque borders).

Central Centrifugal Cicatricial Alopecia (CCCA) detection — flag in photoNote when visible:
CCCA is the most common primary scarring (cicatricial) alopecia in Black women, though it can affect people of any ethnicity with naturally textured hair. It is driven by progressive perifollicular inflammation that permanently destroys follicles — unlike AGA, it is not androgen-driven, and DHT suppressants alone are inadequate. Early identification is critical: scarring is irreversible, and anti-inflammatory treatment significantly slows progression. Key visual distinguishers from AGA:
- Location and geometry: CCCA begins at the crown vertex (the central top of the head) and expands centrifugally — outward in a circle from the center. Unlike AGA (which recedes the frontotemporal hairline inward or spreads from the vertex outward along a Norwood pattern) or female-pattern AGA (which widens along the central part from front to back without a discrete bald patch), CCCA produces a distinctly circular or ovoid bald or severely thinned zone centered on the vertex that grows radially outward. This "bull's-eye" pattern — a bald or sparse center at the vertex, surrounded by a transitional zone of thinning hair, with intact lateral and frontal hair beyond — is the defining feature.
- Hairline preservation: CCCA typically preserves the frontal hairline and temple hair through most of the disease course, even when crown loss is substantial. A clearly intact frontal hairline with a prominent crown vertex bald zone strongly favors CCCA over AGA. At an equivalent crown loss severity, AGA would show at least some frontotemporal hairline recession (NW3v or NW4-equivalent temple retreat), whereas CCCA can produce a large crown bald patch while the frontal hairline remains at the forehead crease.
- Scalp surface texture at the loss boundary: the advancing edge of the CCCA bald zone may show perifollicular changes — subtle erythema (redness) or peripilar scale at the transitional boundary between the bald center and adjacent hair, often appearing as a slightly rough or inflamed rim. These are difficult to see in most photos but should be noted when visible. The surface of the bald zone itself may look smooth but slightly shiny compared to AGA bald skin.
- Broken hairs / rough transitional edge: the transitional zone between the bald center and the intact peripheral hair may show short broken hair shafts or a rough-edged border rather than a gradual smooth density fade — a sign of active follicle destruction at the margin.
- Who it affects: predominantly Black women (prevalence estimates range from 3% to over 50% in some epidemiological studies depending on population studied), but also Black men and people of other ethnicities with naturally textured (Type 3–4) Afro-textured hair. A history of chemical relaxers, chronic heat styling, or tight hairstyles may contribute. CCCA can co-occur with traction alopecia at the hairline — if both patterns are visible, flag both.
When a crown-centered, centrifugally expanding bald zone is visible (a circular or ovoid bald patch or severe thinning at the vertex, NOT at the temples or hairline) AND the frontal hairline is clearly more intact than would be expected at the equivalent crown loss severity under AGA staging AND the user appears to have naturally textured (Type 3–4) Afro-textured hair or presents as Black or Afro-descendant: flag this in photoNote with a note such as: "Crown-centered hair loss expanding outward from the vertex with a relatively preserved frontal hairline — this pattern may be consistent with Central Centrifugal Cicatricial Alopecia (CCCA), a progressive scarring alopecia most common in Black women that begins at the crown vertex and expands centrifugally. CCCA causes irreversible follicle scarring and requires anti-inflammatory treatment (intralesional corticosteroids, topical corticosteroids, often combined with doxycycline, hydroxychloroquine, or 5-alpha reductase inhibitors as adjuncts) — standard AGA minoxidil and DHT-blocking shampoo alone are not adequate. Early diagnosis significantly slows progression; a dermatologist or trichologist familiar with CCCA is the most important next step." Still assign the closest AGA stage for the app's scoring system — use 'n/a (female)' for female users (crown vertex loss maps to Ludwig II-III equivalent) or the appropriate NW stage for male users. Do NOT flag routine AGA crown thinning that follows the standard Norwood progression (crown loss that co-exists with temple recession at NW3v/NW4, spreading from both zones simultaneously) as CCCA — only flag when the crown-centered vertex bald patch is the PRIMARY loss zone with the frontal hairline clearly MORE intact than expected for the assessed AGA stage. Do NOT apply CCCA suspicion to photos where the crown vertex cannot be seen (frontal-only selfies), since CCCA is a diagnosis requiring a clear overhead or angled view of the vertex. Do NOT flag to any user with straight or mildly wavy hair unless a centrifugal crown bald pattern with intact frontal hairline is unmistakably present.

Lichen Planopilaris (LPP) / scarring alopecia detection — flag in photoNote when visible:
Lichen Planopilaris (LPP) is a primary lymphocytic scarring (cicatricial) alopecia and one of the most common causes of permanent hair loss in White and Asian women, though it affects all ethnicities and both sexes. Like FFA and CCCA, LPP permanently destroys follicles through perifollicular inflammation — it is NOT androgen-driven and does NOT respond to standard AGA treatments (minoxidil, finasteride) as primary therapy. LPP is frequently misidentified as AGA crown thinning or diffuse loss, leading to missed diagnosis and delayed treatment. Key visual distinguishers from AGA and other conditions:
- Pattern geometry: LPP produces IRREGULAR, ASYMMETRIC, multifocal patchy hair loss that does NOT follow Norwood progression geometry — the bald patches do not respect the temple-recession → crown-thinning progression of AGA, do not form the uniform frontotemporal band of FFA, and do not expand centrifugally from the vertex like CCCA. Instead, LPP creates scattered irregular loss zones across the vertex, parietal scalp, or any crown region that appears geometrically inconsistent with standard AGA stage anatomy.
- Active border signs: the leading edge of active LPP patches may show perifollicular erythema (redness around individual follicle openings) or perifollicular scale (whitish or follicle-centered scale at the margin of the bald zone, similar in character to FFA peripilar casts). These signs indicate active inflammation destroying new follicles at the patch margin.
- Bald zone surface texture: within established LPP patches, the scalp surface shows complete absence of visible follicle openings — the skin appears smooth and slightly shiny, distinguishing it from AA (alopecia areata, which preserves follicle openings as small pores visible in the bald zone) and from AGA (which shows miniaturized vellus hairs in thinning zones, not a completely smooth bald surface).
- Hairline preserved: unlike FFA (which recedes the frontotemporal margin) and unlike AGA (which recedes temples and/or crown in a Norwood pattern), LPP typically PRESERVES the frontal hairline and frontotemporal margin through most of the disease course, even when vertex/parietal patches are large. This is a key clinical signal: significant parietal or vertex scarring with a fully intact frontal hairline favors LPP over AGA.
- Distinguishing from CCCA: CCCA has a specific centrifugal bull's-eye pattern centered on the vertex, predominantly in Black women with textured hair. LPP is geometrically irregular (not centrifugal), more common in White/Asian women, and can involve any crown or parietal region rather than specifically radiating from the vertex center.
- Distinguishing from AA (alopecia areata): AA patches are perfectly smooth, completely bald oval zones with preserved follicle openings (tiny pores visible within the bald area) — the bald skin looks normal except for complete absence of hair. LPP patches may show active perifollicular scale or erythema at the advancing margin, and established LPP patches show permanently absent follicle openings (smooth, shiny, non-porous skin). AA is also non-scarring; LPP is scarring.
- Who it affects: most common in women aged 40–70 of White, Mediterranean, or Asian ancestry, but increasingly recognized in younger women and in men. No specific hairstyle trigger (unlike CCCA and TA). Can co-exist with FFA in some patients (vertex LPP + frontotemporal FFA band).
When IRREGULAR, GEOMETRICALLY ASYMMETRIC patchy hair loss is visible at the vertex, parietal scalp, or crown that DOES NOT follow standard Norwood progression geometry (not M-shape temple recession, not centrifugal from vertex, not diffuse uniform thinning) AND the frontal hairline is clearly preserved AND the bald zone surface appears smooth (with possible perifollicular erythema or scale at the patch margin): flag this in photoNote with a note such as: "Irregular patchy hair loss at the crown or parietal scalp that doesn't follow the typical Norwood progression pattern — this may be consistent with Lichen Planopilaris (LPP), a primary scarring alopecia that causes permanent follicle destruction through perifollicular inflammation. LPP requires anti-inflammatory treatment (hydroxychloroquine, cyclosporine, or intralesional/topical corticosteroids) — standard AGA topicals (minoxidil) and DHT blockers alone are not adequate as primary therapy. Scarring is irreversible and early diagnosis significantly slows progression; a dermatologist or trichologist is the most important next step to confirm the diagnosis before starting treatment." Still assign the closest AGA stage for the app's scoring system (use the NW stage or diffuse/n/a female that most closely matches the visible coverage extent, acknowledging the non-AGA pattern in photoNote). Do NOT flag standard AGA crown thinning that co-exists with temple recession at NW3v/NW4 as LPP — only flag when the patchy loss geometry is clearly INCONSISTENT with expected Norwood anatomy. Do NOT flag CCCA (centrifugal bull's-eye from vertex in Black women) as LPP — the centrifugal geometry and ethnicity pattern are distinctive. Do NOT flag alopecia areata (AA) patches as LPP — AA patches are smooth, round/oval, and non-scarring; if round/oval patches are the dominant feature, use the AA detection rule instead. Do NOT apply LPP suspicion to photos where only a frontal selfie view is available with no crown or parietal region visible. Apply LPP suspicion with HIGH confidence only when the irregular multifocal patchy geometry is unmistakably inconsistent with AGA and the frontotemporal hairline is clearly intact.

Scoring guide (all scores 0-100 integers):
- hairline: 100 = fully intact hairline, no recession. Deduct for temple recession depth/width, frontal loss. Stage-correlated ranges: NW1→90-100, NW2→75-88, NW3→55-72, NW3v→55-72 (same temple recession depth as NW3; vertex thinning that defines NW3v does not affect the hairline position or temple recession depth — score within the NW3 hairline range), NW4→35-55, NW5→20-38 (frontal and crown nearly merged, minimal forelock), NW6→10-25 (frontal and crown merged; only lateral fringe; hairline score reflects fringe height), NW7→5-18 (horseshoe fringe only; near-total frontal loss), diffuse→65-88 (diffuse thinning preserves the hairline position and temple shape — the key distinguisher from NW2/NW3 is the ABSENCE of M-shape temple recession; score 78-88 when the hairline is fully intact with no visible temple angle; score 65-78 when mild frontal density reduction is visible but the temple geometry remains non-angular), n/a (female)→72-95 (female-pattern hair loss almost always spares the frontal hairline — score 82-95 when the hairline is fully intact regardless of crown/part thinning; score 72-82 only if diffuse frontal hairline thinning is clearly visible, e.g. Ludwig III frontal type). Close-up selfie / wide-angle lens distortion calibration: close-range selfies taken with the front-facing (selfie) camera — especially those where the face fills the entire frame or the ears are clipped at the frame edges — exhibit barrel distortion from the wide-angle lens, which pushes the outer temple corners laterally toward the frame edges and makes the forehead appear wider than its actual physical dimensions. This distortion displaces the apparent position of the frontotemporal hairline outward, making temple recession look deeper than it actually is. Visual signs: face substantially fills the frame, ears partially or fully cut off, head appears unusually wide relative to its height, forehead looks unnaturally broad. When these signs are present and the temple recession appears to be at the NW2/NW3 boundary: (a) apply a +5 to +8 point upward correction to the hairline score to compensate for distortion-inflated apparent recession depth; (b) do NOT automatically assign NW3 when the recession appears borderline — treat it as NW2 unless Concerns explicitly confirm deeper recession, since the distortion can displace a true NW2 temple corner past the mid-pupil vertical reference; (c) note the wide-angle distortion in photoNote and recommend a retake from ~50cm away with the rear camera or from a slightly greater distance to minimize lens distortion.
- density: 100 = full terminal hair density with no scalp visible through hair. Deduct for mid-scalp see-through, diffuse thinning, miniaturization. Stage-correlated ranges: NW1→88-100, NW2→80-95, NW3→65-82 (reduce by 5-15 more if visible mid-scalp thinning alongside the recession), NW3v→58-78 (early vertex thinning at NW3v typically coincides with some mid-scalp density reduction; default to the lower half of the NW3 range; reduce toward 55-65 if visible mid-scalp see-through or miniaturization is present alongside the temple recession), NW4→45-68, NW5→30-52, NW6→12-35 (large bald top; only lateral fringe provides density), NW7→8-25 (near-total top loss; fringe hairs may be thin themselves), diffuse→35-65 (depends on coverage uniformity), n/a (female)→30-78 (Ludwig I mild central-part widening→65-78; Ludwig II moderate diffuse thinning→48-65; Ludwig III severe thinning→30-48; score based on how much scalp is visible through the central part and crown region). If the photo is a straight-on face shot where mid-scalp isn't visible, estimate from stage. Hair color calibration: scalp visibility through light-colored hair (blonde, grey, white, silver) is naturally higher than through dark hair at the same actual density level. Assess coverage by estimating the percentage of scalp that has hair over it, not purely by how much bare scalp shows through — a person with grey or blonde hair whose strands cover 90% of the scalp zone should score similarly to a dark-haired person with the same coverage. Do not deduct density points simply because scalp is more visible through lighter hair pigment. Very short hair calibration: hair cut to ≤6mm (buzz cut, #1-#2 clipper guard, or machine shave) will naturally show significant scalp visibility between follicles because there is no shaft length to bridge the gaps — this is a physical consequence of hair length, not a sign of thinning. For very short hair, assess density by evaluating the uniformity of follicle distribution across the scalp top; score 78-95 for a uniform clipper cut with no visible bald patches or density differential between zones. Do not deduct density points because a buzz cut shows more scalp than the same person with longer hair would. Wet hair / styling product calibration: wet hair (post-shower, post-swim) and hair loaded with gel, pomade, wax, or heavy mousse clumps into rope-like groups of strands, exposing scalp between the clumps even when follicle density is healthy. This can make density appear 10-20 points lower than it actually is. When wet-hair texture (dark, slick, strand-clumping with visible wet shine) or heavy-product clumping is visible, assess density from the uniformity and coverage of follicle groups across the scalp zone rather than from raw scalp-show-through between wet clumps. Do not deduct density points solely because wet or product-heavy clumping reveals scalp between strand groups; use the visible follicle distribution to estimate what dry coverage would look like. Always note the wet/styled condition in photoNote and recommend the user retake the photo with dry, unstyled hair for the most accurate reading. Hair building fiber / concealer calibration: cosmetic density products — keratin fiber sprays (Toppik, Caboki, Bumble and bumble Fill-In Powder, Boldify, Nanogen), scalp concealers (DermMatch), and spray-on root concealers — add artificial bulk to sparse zones, making density appear significantly higher than the actual underlying follicle coverage. Visual signs of fiber product use: unusually even or thick coverage in zones where the assessed Norwood stage predicts visible scalp; fine speckling, powdery, or gritty texture visible on the scalp surface or near hair roots; or density that looks inconsistent with the degree of recession or hairline loss visible at the scalp edge. When fiber or concealer use is strongly suspected: (a) score density 15–25 points lower than the raw photo appearance to estimate actual underlying follicle coverage; (b) rate photoQuality as 'acceptable'; (c) note the suspected fiber/concealer use in photoNote and recommend a retake on dry, product-free scalp for accurate scores. Flash photography / overexposure calibration: direct smartphone flash at close range creates a bright overexposed area on the scalp surface that artificially inflates scalp visibility between hairs, making density appear 10–20 points lower than the actual underlying follicle coverage. Visual signs of direct flash: a central or circular overexposed bright patch where the scalp looks unnaturally white or washed out between hair strands; strong specular highlights along individual hair shafts; surrounding hair zones appear darker by contrast. When direct-flash overexposure is visible on the scalp, assess density from follicle distribution uniformity across the zone rather than from raw scalp-show-through in the overexposed area. Do not deduct density points for flash-induced glare; rate photoQuality as 'acceptable' and note the flash lighting condition in photoNote, recommending the user retake in natural or diffuse indoor lighting for the most accurate reading. Scalp-skin contrast calibration: scalp visibility between hair strands is affected by the contrast between hair color and skin tone. Users with dark hair against pale or light skin (Fitzpatrick types I–II) show high scalp-hair contrast — individual strands are sharply delineated against a bright scalp surface, making inter-strand gaps more visually prominent even at moderate underlying density. Users with dark hair against darker skin tones (Fitzpatrick types IV–VI) show lower scalp-hair contrast — inter-strand gaps are less visually prominent even at similar underlying coverage levels. When assessing density for high-contrast (pale skin + dark hair) users: assess coverage from follicle spacing uniformity across the scalp zone rather than raw scalp-show-through, and do not penalize density solely because high contrast makes individual gaps more visible. For lower-contrast (dark skin + dark hair) users: do not inflate density scores based on reduced visible scalp — assess follicle coverage patterns the same way. The goal is an equitable density score that reflects actual follicle count and coverage uniformity, independent of skin-hair color contrast. Comb-over / hair-draping calibration: some users sweep long strands from a lower-density zone (typically the sides or back where growth is intact) laterally or forward over a sparse or bald zone (typically the crown, vertex, or mid-scalp) to camouflage underlying loss. Visual signs: long hair on one or both sides angled across the scalp top in an atypical direction; a visible 'landing edge' or abrupt coverage transition; scalp visibility at the sweep origin or below a lifted strand layer; the apparent coverage pattern inconsistent with the stated recession stage (e.g. heavily receded hairline yet apparently full crown coverage). When a clear comb-over or draping pattern is visible: (a) score density 15–25 points lower than the raw apparent surface coverage to estimate actual underlying follicle coverage in the draped zone; (b) rate photoQuality as 'acceptable'; (c) note the suspected comb-over or hair-draping technique in photoNote and recommend the user retake the photo with hair in its natural fall (no side sweeping) for accurate scoring. When draping is possible but uncertain, apply a conservative 8–12 point discount from apparent surface coverage and still note it in photoNote. Scalp micropigmentation (SMP) calibration: SMP is a cosmetic tattooing procedure that deposits uniform ink dots onto the scalp surface to simulate the look of very short hair follicles and camouflage bald or thinning zones. Visual signs of SMP: (a) perfectly uniform stippled dots of identical size and spacing across a zone the assessed stage would predict is bald — real hair, even at ≤6mm buzz-cut length, shows natural variation in follicle direction, spacing, and slight shaft protrusion above the skin, while SMP dots are flat, pixel-perfect, and protrude nothing; (b) unnaturally even pigment distribution with no directional grain or follicle-tip shine; (c) the stippled zone may have a slightly different texture, matte finish, or blue-grey/dark tint compared to adjacent natural scalp skin or the lateral fringe's real hair. When SMP is strongly suspected: (a) score density based on the UNDERLYING scalp extent — treat the SMP'd zone as bald scalp, not as follicle coverage; estimate underlying density from the stage-appropriate range for the true loss level (typically NW5–NW7 when a large crown or top zone is SMP'd); (b) rate photoQuality as 'acceptable'; (c) note the suspected SMP in photoNote and recommend a retake in clear natural lighting that reveals the real-hair vs. tattooed-area boundary. When unsure whether a uniform short-hair appearance is SMP vs. a real clipper cut (they can look similar), apply only a conservative 5–10 point discount and defer to the real buzz-cut calibration (78–95 for uniform clipper cuts) rather than the full SMP override — SMP is a confident call, not a default assumption. Tightly coiled / Afro-textured hair calibration: tightly coiled hair (Type 4 natural hair, afro texture, TWA — teeny weeny afro) has a distinctive spiral curl pattern where each strand loops close to the scalp surface, creating a raised carpet-like coverage. Two effects matter for density scoring: (a) visible scalp between individual coil loops is normal at healthy follicle density — the spring shape of each strand exposes scalp beneath the curl even at 80–90%+ underlying coverage. Do NOT deduct density points for between-coil scalp visibility in tightly coiled hair; assess coverage from how densely coil clusters are distributed across the scalp zone, not from raw scalp-show-through between individual curl loops. Score 78–95 when coil clusters are uniformly distributed with no visible bald patches or clear density differential between zones. (b) Thinning in afro-textured hair typically presents as a visibly widened part line, a distinct circular bald patch at the crown vertex, or noticeably thinner / smaller coil clusters in a focal zone — NOT as generalized scalp visibility between coils. Identify thinning from coil distribution asymmetry or a focal sparse zone, not from the presence of between-coil scalp visibility, which is baseline normal for this hair type. Wavy / curly hair calibration (Type 2–3): medium-wave (Type 2, S-wave / beach wave) and curly (Type 3, spiral curl 3a–3c) hair naturally inflates apparent density because the wave or curl arches away from the scalp, reducing visible scalp between strands compared to straight hair at the same actual follicle coverage. This inflation is especially prominent at the crown and mid-scalp where the curved hair mass forms a rounded canopy. (a) Density inflation: when hair shows a clear S-wave, beach-wave, or defined spiral curl pattern, apply a conservative 5–10 point discount from the apparent surface-coverage density to estimate actual underlying follicle count; use the lower end of the discount for mild Type 2 waves and the upper end for pronounced Type 3 spirals. (b) Thinning detection: actual thinning in wavy/curly hair presents as a visible break or gap in the wave/curl canopy, a zone where strands fall flat and straight against the scalp rather than curling, or a noticeably thinner/smaller curl cluster field with reduced canopy lift in a focal zone; a widened part line is a reliable thinning signal regardless of wave or curl pattern. Do NOT deduct density points simply because a wave or curl pattern creates visible separation between curl clusters when the overall canopy appears uniformly lifted and distributed across the scalp zone. Outdoor harsh direct sunlight calibration: harsh overhead or angled direct sunlight (midday cloudless sky, unshaded outdoor selfie) creates hard, narrow shadows at the base of each hair strand against the scalp surface, visually exaggerating scalp show-through and making density appear 5–15 points lower than the actual underlying follicle coverage. Visual signs of harsh direct sunlight: sharply defined dark shadows radiating from hair strand bases against a bright scalp; high-contrast bright-scalp / dark-shadow alternating pattern between hairs without the central overexposed halo characteristic of indoor flash; overall high-contrast, high-brightness photo with harsh directional light. When harsh direct-sunlight shadow patterns are visible, assess density from the overall distribution of hair mass across the scalp zone rather than from the shadow-expanded bare-scalp impression; do not deduct density points for optical shadow expansion caused by direct sun. Rate photoQuality as 'acceptable' and note harsh outdoor sunlight in photoNote, recommending a retake in open shade, diffuse cloud cover, or soft indoor light for the most accurate reading. Backlit photo calibration: when the primary light source (sun, bright window, open sky) is positioned behind the user, hair is rim-lit from behind, creating a bright halo or glow around the hair perimeter — the hair mass appears fuller and more voluminous than it actually is, potentially masking thinning zones. Visual signs of backlighting: bright rim or halo of light at the outer hair perimeter; the hair mass itself appears darker against a bright background; scalp and hairline detail within the hair mass is hard to distinguish; hairline edges may look less receded than they actually are. When strong backlighting is present, apply a conservative 8–12 point discount from the apparent density coverage to compensate for the volume-inflation effect; rate photoQuality as 'acceptable' and note the backlit condition in photoNote, recommending a retake with the light source in front of or above the user (not behind) for an accurate baseline.
- crown: 100 = full vertex/crown coverage from above. Stage-correlated ranges: NW1→90-100, NW2→87-100, NW3→82-97 (crown should be intact unless NW3v), NW3v→55-75 (early vertex thinning distinguishes this stage), NW4→35-58, NW5→18-40, NW6→5-20 (crown fully merged with frontal zone; essentially no vertex coverage), NW7→2-12 (complete vertex absence; only narrow horseshoe fringe survives), diffuse→32-72 (crown involvement varies with severity; mild diffuse AGA with intact vertex coverage→60-72; moderate diffuse with visible scalp through the vertex and crown zone→45-60; severe diffuse with widespread vertex thinning and large exposed crown area→32-45), n/a (female)→22-88 (Ludwig I: crown mostly intact, only the part line is wider→72-88; Ludwig II: crown thinning visible from above→48-72; Ludwig III: severe vertex and crown loss with large exposed area→22-48). If crown IS visible in the photo, score directly from what you see. If not, use the stage estimate. Flash photography / overexposure calibration from above: direct smartphone flash aimed at the top of the scalp creates a central overexposed bright patch in overhead shots that makes the bare crown area appear larger than it actually is — the overexposed zone looks unnaturally white or washed out, and the boundary between covered and bare scalp may fade into the bright flash zone rather than showing a sharp, clean edge. This can make crown coverage appear 5-15 points lower than the actual underlying follicle coverage. When a central overexposed bright patch is visible in an overhead shot, assess the covered-to-bare boundary from the follicle distribution visible at the edges of the bright region — do not use the apparent size of the overexposed zone itself as the bald area boundary. Do not deduct crown points for flash-induced glare in overhead shots; rate photoQuality as 'acceptable' and note the flash lighting condition in photoNote, recommending the user retake from the same overhead angle in natural or diffuse indoor lighting for the most accurate reading. Wet hair / styling product calibration from above: wet hair (post-shower, post-swim) and hair loaded with gel, pomade, wax, or heavy mousse clumps into rope-like groups of strands, exposing scalp between the clumps even when follicle density is healthy — this effect is especially pronounced in overhead shots where the full crown is in view. This can make crown coverage appear 8-15 points lower than the actual underlying follicle coverage. When wet-hair texture (dark, slick, strand-clumping with visible wet shine) or heavy-product clumping is visible in an overhead shot, assess crown coverage from the uniformity of follicle group distribution across the vertex region rather than from raw scalp-show-through between wet clumps. Do not deduct crown points solely because wet or product-heavy clumping reveals scalp between strand groups; use the visible follicle cluster spacing to estimate what dry coverage would look like. When wet or styled conditions are already noted for the density score, the same artifact affects crown coverage assessment in overhead shots — apply the same conservative approach and note the condition in photoNote if not already noted. Scalp micropigmentation (SMP) calibration from above: in overhead crown shots, SMP dots can closely resemble a shaved crown, potentially masking a large bald vertex area entirely. Visual signs in an overhead shot: perfectly uniform stippled dots across the crown/vertex zone where the assessed stage predicts bald scalp; no shaft-tip shine or protrusion visible even at high detail (real buzz cuts show tiny terminal shaft tips and natural spacing variation); the SMP'd zone boundary may show an abrupt transition to the genuine natural hair of the lateral fringe. When SMP is strongly suspected in an overhead crown shot: score crown coverage from the underlying bald extent (treat SMP'd area as bald), placing the result within the NW5–NW7 crown range (5–40) rather than using the visual surface appearance; rate photoQuality as 'acceptable'; note suspected SMP in photoNote. When unsure whether the crown appearance is SMP vs. a real clipper cut, apply a conservative 5–10 point discount and do not fully override — buzz-cut-at-the-crown is common and the conservative buzz-cut calibration takes precedence when SMP is not confidently identified. Tightly coiled / Afro-textured crown calibration: in overhead shots of tightly coiled / afro-textured hair, the raised curl profile creates a dome-like coverage over the crown vertex — scalp visible between coil bases at the vertex is normal baseline for this hair type and should NOT be scored as crown thinning. When a crown bald patch IS present in tightly coiled hair it typically appears as a distinct, clearly delineated circular or oval zone where coil clusters are absent or noticeably sparse, with a visible contrast to the surrounding coil field. Score crown from the extent of coil cluster coverage across the vertex zone; do not deduct crown points for between-coil scalp gaps in an otherwise uniformly distributed coil field. Wavy / curly hair crown calibration (Type 2–3): in overhead shots, the arching wave or curl canopy at the crown vertex may appear to cover more scalp than actually exists beneath it — curls arch away from the scalp and partially obscure the vertex surface, making the crown look fuller than the underlying follicle coverage warrants. Apply a conservative 5–10 point discount from the apparent crown coverage when a clear wavy or curly hair texture is visible in an overhead shot (Type 2 wave: lower end; Type 3 curl: upper end). Thinning in wavy/curly hair at the crown typically presents as a zone where the curl or wave pattern collapses — a flat, straight-falling area surrounded by a curly canopy elsewhere, or a visibly sparser curl cluster field at the vertex compared to the surrounding crown region. Do not deduct crown points solely because curl clusters naturally separate from each other when the vertex canopy appears uniformly lifted and distributed. Outdoor harsh direct sunlight calibration from above: in overhead shots taken under harsh direct sunlight, the same hard-shadow artifact applies to crown assessment — shadows at the base of each strand expand the apparent bare-scalp area at the vertex, making crown coverage appear 5–15 points lower than the actual follicle coverage. Assess crown from the uniformity of hair mass distribution across the vertex zone rather than from the shadow-expanded appearance of the bare area; do not deduct crown points for shadow-exaggerated scalp exposure. Rate photoQuality as 'acceptable' and note harsh outdoor lighting in photoNote if not already noted. Backlit photo calibration from above: in overhead shots with a bright sky or sun directly above and behind the camera, the backlit halo can inflate apparent crown coverage and partially obscure the true extent of the bald zone at the vertex — the bright rim blends the hair perimeter with the background, making the covered crown area look larger than it is. Apply a conservative 8–10 point discount from the apparent crown coverage when strong backlighting is visible in an overhead shot; note the backlit condition in photoNote if not already noted.
- health: 100 = thick terminal hairs at normal caliber, healthy scalp, no miniaturization or inflammation. Deduct for: visible miniaturization (fine, short hairs at the thinning edge) −10 to −20; scalp inflammation or redness −5 to −15; visible flakiness or dandruff −5 to −10; widespread vellus hairs replacing terminal hairs −10 to −20. Health is NOT determined by hairline position — a NW4 with thick terminal hairs and healthy scalp can score high on health. Stage-correlated baseline ranges (adjust up/down based on visible scalp condition): NW1→82-100, NW2→75-95, NW3→65-85, NW3v→60-82, NW4→55-80 (still high if remaining hair is thick and scalp healthy), NW5→45-70, NW6→38-62 (lateral fringe may still be healthy terminal hair), NW7→30-55 (even fringe hairs may be finer at this stage), diffuse→35-70 (cause-dependent; often inflammatory or nutritional), n/a (female)→45-78 (often nutritional/hormonal; health improves when root cause treated). Stress/TE calibration: when Stress ≥ 7/10 in the user profile context and the visible pattern is diffuse uniform thinning without discrete miniaturization (individual hairs look normal in shaft thickness despite widespread shedding), telogen effluvium (TE) is likely co-driving or primarily causing the loss alongside AGA. TE preserves individual follicle shaft caliber — the follicles themselves are healthy, only the growth cycle timing is disrupted. When Stress ≥ 7 and diffuse thinning appears with normal-looking individual shaft thickness, score health toward the upper end of the diffuse or n/a(female) range (55-70 for diffuse, 62-78 for female pattern) because underlying follicle integrity is intact; do NOT apply miniaturization deductions unless visibly finer vellus hairs are present. Likewise, if Sleep ≤ 5h and a diffuse shedding pattern is visible, apply the same lenient health calibration (chronic sleep deprivation is a recognized TE trigger). Grey/white hair calibration: grey and white hair is naturally slightly finer in shaft diameter than pigmented hair — this is a normal age-related characteristic, not miniaturization. Deduct health points for miniaturization only when there is a visible contrast between clearly full-caliber hairs and noticeably shorter, wispier vellus hairs in the same zone. Do not automatically penalize older users with uniformly grey or white hair for the natural fineness of depigmented strands. Very short hair calibration: with a buzz cut or clipper cut (≤6mm), all terminal hairs appear short and may look similar in caliber to vellus hairs simply because they are trimmed close to the scalp. Score miniaturization only when there is a visible spatial contrast — areas where hairs are clearly shorter, wispier, or sparser than the surrounding uniformly-trimmed hairs (not when the entire cut is uniformly short). Do not deduct health points for a uniform clipper cut where all hairs appear identically trimmed at the same length with no visible density differential. Hair building fiber / concealer calibration: keratin fibers and scalp concealers coat the hair shaft and scalp surface, making miniaturization assessment unreliable in covered zones — fine or vellus hairs are hidden beneath the fiber coating. When fiber products are suspected (see density calibration above), do not apply aggressive miniaturization deductions to zones that appear covered; note in photoNote that health/miniaturization assessment may be imprecise in product-covered areas. Flash photography / overexposure calibration: direct smartphone flash overexposes the scalp surface and washes out fine hair shaft detail, making individual full-caliber terminal hairs appear thinner or more vellus-like than they actually are. This is an optical artifact of overexposure, not genuine miniaturization. Do not apply miniaturization deductions based on apparent shaft thinning in flash-overexposed zones; if flash signs are present (see density calibration above), note the flash condition in photoNote and treat miniaturization assessment in overexposed zones as unreliable. Wet hair / styling product calibration: wet hair (post-shower, post-swim) and hair heavily coated with gel, pomade, or wax make miniaturization assessment unreliable — water weight and product residue flatten individual hair shafts against the scalp and reduce apparent shaft diameter, causing healthy full-caliber terminal hairs to appear thin or wispier than they actually are, mimicking vellus-like fineness. This is a physical artifact of moisture and product load, not genuine miniaturization. When wet-hair texture (dark, slick, strand-clumping with visible wet shine) or heavy product coating is visible, do not apply miniaturization or shaft-caliber deductions in the wet/product-loaded zones; assess health instead from visible scalp condition (redness, flakiness, inflammation) and the uniformity of follicle distribution across the scalp zone. When wet or styled conditions are already noted for the density score (above), the same artifact makes health-metric shaft-caliber assessment unreliable in those same zones — use the same conservative approach and note the condition in photoNote if not already noted. Minoxidil-onset shedding calibration: when 'Current routine' includes minoxidil (topical or oral, including any brand or generic) AND the onset field indicates a recent start (≤6 months, 'recently started', '1-2 months', '3 months', '3-6 months', or equivalent) OR the user's Concerns include 'shedding', 'more hair falling', 'hair falling more', 'worse since starting', or 'increased shedding since starting', visible diffuse shedding may be treatment-induced telogen effluvium — a normal, expected phase occurring 4–12 weeks after minoxidil initiation as follicles shift from telogen to anagen phase. Treatment-onset TE is self-limiting (typically resolves by month 3–5) and indicates that follicles are actively responding to treatment, not deteriorating. When this pattern is plausible (minoxidil in routine + onset ≤6 months + shedding appears diffuse rather than focal): (a) do NOT deduct health points for visible diffuse shedding if the remaining hairs appear normal-caliber without obvious vellus miniaturization — shaft caliber integrity is what distinguishes TE from AGA progression; (b) score health toward the upper half of the stage range because underlying follicle integrity is intact; (c) note in photoNote that diffuse shedding alongside a recent minoxidil start may represent normal treatment-induced TE that typically self-resolves by month 4–5, and recommend continuing consistent daily application. Do NOT apply this calibration when shedding is clearly focal (a defined temple recession or crown bald patch with sharp edges) — focal androgenic loss indicates miniaturization, not TE, and should be scored normally. Finasteride-onset shedding calibration: when 'Current routine' includes finasteride (or any finasteride-containing product — Propecia, Proscar, Finpecia, Finalo, Fincar, Finax, Finast, Aindeem, or a topical combo like Morr-F, Tugain-F, or Finalo-F containing 0.1% finasteride) AND the onset field indicates a recent start (≤6 months, 'recently started', '1-2 months', '3 months', '3-6 months', or equivalent) OR the user's Concerns include 'shedding', 'increased shedding', 'more hair falling since starting', or 'worse since starting finasteride', visible diffuse shedding may be finasteride-onset telogen effluvium — occurring in approximately 2–5% of users within the first 1–4 months as DHT suppression briefly disrupts the existing hair growth cycle, temporarily synchronizing follicles into telogen before restabilizing in anagen. Like minoxidil-onset TE, this is self-limiting (typically resolves by month 3–6) and does not indicate AGA progression — it is a sign the follicles are responding to systemic DHT reduction. When finasteride-onset TE is plausible (finasteride in routine + recent onset + shedding appears diffuse rather than focal): (a) do NOT deduct health points for visible diffuse shedding if the remaining hairs appear normal-caliber without obvious vellus miniaturization — shaft caliber integrity is the key distinguisher between TE and AGA progression; (b) score health toward the upper half of the stage range because underlying follicle integrity is intact; (c) note in photoNote that diffuse shedding alongside a recent finasteride start may represent normal treatment-induced TE that typically self-resolves by month 4–6, and recommend continuing consistent daily use. Do NOT apply when shedding is clearly focal. Dutasteride-onset shedding calibration: when 'Current routine' includes dutasteride (Avodart, Dutas, Duprost, Duodart, or any dutasteride-containing brand or generic) AND the onset field indicates a recent start (≤6 months, 'recently started', '1-2 months', '3 months', '3-6 months', or equivalent) OR the user's Concerns include 'shedding', 'increased shedding', 'more hair falling since starting', or 'worse since starting dutasteride', visible diffuse shedding may be dutasteride-onset telogen effluvium — occurring within the first 1–4 months as the potent dual 5-alpha reductase inhibition (types 1 and 2) sharply reduces scalp DHT, temporarily synchronizing follicles into telogen before restabilizing in anagen. Dutasteride's long half-life (approximately 5 weeks) produces deeper, more sustained DHT suppression than finasteride; the resulting TE shedding phase follows the same mechanism and may be slightly more pronounced or last slightly longer in some users (resolving by month 5–7 rather than 4–6). This is self-limiting and does not indicate AGA progression. When dutasteride-onset TE is plausible (dutasteride in routine + recent onset + shedding appears diffuse rather than focal): (a) do NOT deduct health points for visible diffuse shedding if the remaining hairs appear normal-caliber without obvious vellus miniaturization — shaft caliber integrity distinguishes TE from AGA progression; (b) score health toward the upper half of the stage range because underlying follicle integrity is intact; (c) note in photoNote that diffuse shedding alongside a recent dutasteride start may represent normal treatment-induced TE that typically self-resolves by month 5–7, and recommend continuing consistent daily use. Do NOT apply when shedding is clearly focal. Spironolactone-onset shedding calibration: when 'Current routine' includes spironolactone (Aldactone, or any spironolactone-containing product) AND the onset field indicates a recent start (≤6 months) OR the user's Concerns include shedding or increased hair loss since starting, visible diffuse shedding may be spironolactone-onset telogen effluvium — a hormonal adjustment response occurring in some users within the first 1–4 months as the antiandrogen effect modulates the androgen signaling that regulates hair follicle cycling. It is self-limiting (typically resolves by month 3–5) and does not indicate AGA progression. When spironolactone-onset TE is plausible (spironolactone in routine + onset ≤6 months + shedding appears diffuse rather than focal): (a) do NOT deduct health points for visible diffuse shedding if the remaining hairs appear normal-caliber without obvious vellus miniaturization — shaft caliber integrity distinguishes TE from AGA progression; (b) score health toward the upper half of the stage-appropriate range because underlying follicle integrity is intact; (c) note in photoNote that diffuse shedding alongside a recent spironolactone start may represent a normal hormonal adjustment TE response that typically self-resolves by month 3–5, and recommend continuing consistent use. Do NOT apply when stage is NW1-NW7 AGA with clear androgenic recession — apply only for diffuse or n/a (female) pattern. Do NOT apply when shedding is clearly focal. Bicalutamide/flutamide/cyproterone-onset shedding calibration: when 'Current routine' includes bicalutamide (Casodex or generic), flutamide (Eulexin or generic), or cyproterone acetate (Androcur or generic) AND the onset field indicates a recent start (≤6 months) OR the user's Concerns include shedding or increased hair loss since starting, visible diffuse shedding may be an androgen-receptor-blocker-onset TE — a hormonal adjustment response that occurs as blockade of androgen receptors modulates the cycle regulation of hair follicles. It is self-limiting (typically resolves by month 2–5) and does not indicate AGA progression. When this pattern is plausible (bicalutamide/flutamide/cyproterone in routine + onset ≤6 months + shedding appears diffuse rather than focal): (a) do NOT deduct health points for visible diffuse shedding if the remaining hairs appear normal-caliber without obvious vellus miniaturization — shaft caliber integrity distinguishes TE from AGA progression; (b) score health toward the upper half of the stage-appropriate range because underlying follicle integrity is intact; (c) note in photoNote that diffuse shedding alongside a recent bicalutamide/flutamide/cyproterone start may represent a normal hormonal adjustment TE response that typically self-resolves by month 3–5, and recommend continuing consistent use. Do NOT apply when stage is NW1-NW7 AGA with clear androgenic recession — apply only for diffuse or n/a (female) pattern. Do NOT apply when shedding is clearly focal. Seasonal shedding calibration: increased diffuse shedding is a well-documented seasonal photoperiod response, most common in late summer to early autumn (August–November in the Northern Hemisphere, February–May in the Southern Hemisphere), peaking 90–120 days after the summer solstice. Seasonal TE is bilateral, diffuse, self-limiting (resolves within 2–3 months), and does NOT indicate AGA progression or treatment failure. Use Today's date in the user profile context to determine whether the user is in the seasonal risk window. When: (1) today's date falls within a seasonal risk month, AND (2) Concerns include shedding, increased hair loss, more hair falling, or similar without a clear AGA progression signal, AND (3) 'Current routine' has been stable for 6+ months with no new medication starts — do NOT deduct health points for diffuse shedding if individual shaft caliber appears normal; score health toward the upper half of the stage-appropriate range, and note in photoNote that seasonal TE may be a plausible contributing factor alongside any AGA context. Do NOT apply this when treatment-onset TE context exists (new minoxidil, finasteride, or dutasteride start within the past 6 months) — treatment-onset TE is a more specific explanation that takes priority. Do NOT apply when the visible pattern shows clear AGA progression (focal temple recession deepening or crown bald-patch enlargement), since that warrants a more urgent action recommendation rather than seasonal reassurance. Postpartum TE shedding calibration: when 'Onset' or 'Concerns' include any postpartum or post-pregnancy indicator — "postpartum", "post-partum", "after baby", "after pregnancy", "after delivery", "after giving birth", "after childbirth", "months after baby", "post-pregnancy", "postnatal", or similar — AND the visible shedding appears diffuse without a clear focal AGA pattern: (a) do NOT deduct health points for visible diffuse shedding if individual shaft caliber appears normal — postpartum TE follicles are completely intact, only the growth cycle timing is disrupted; (b) score health toward the upper half of the stage-appropriate range (62-78 for n/a (female), 58-72 for diffuse); (c) note in photoNote that this diffuse shedding is consistent with postpartum TE, which is self-limiting and typically fully resolves within 6–12 months after delivery without specific hair-loss treatment. Do NOT apply when shedding is clearly focal (defined temple recession or crown bald patch) — that warrants normal AGA scoring. Treatment-onset TE takes priority if both apply.
- potential: realistic percentage improvement achievable with a consistent 6-12 month treatment protocol (OTC or Rx already in the user's "Current routine"). Score what is ACHIEVABLE — not the current state. If "Current routine" contains any Rx antiandrogen — finasteride, dutasteride, spironolactone, bicalutamide, flutamide, cyproterone, or androcur — these drugs substantially improve outcomes at every stage by blocking androgen-driven follicle miniaturization; treat their presence as meaningful upward pressure on potential. Use these stage×age guidelines:
  ...
  PRP upward adjustment: when "Current routine" lists PRP (platelet-rich plasma), PRP injections, or any platelet-rich plasma therapy, apply +4 to +7 on top of the stage baseline — PRP delivers concentrated growth factors directly to follicles, promoting independent regenerative activity that complements the DHT-suppression pathway; studies show 20–30% density improvement at treated sites. Use the higher end (+6 to +7) when PRP is described as ongoing quarterly maintenance or 3+ sessions completed.
  Microneedling/LLLT upward adjustment: when "Current routine" lists microneedling (dermaroller, dermapen, microneedle roller), LLLT (laser cap, Capillus, iRestore, iGrow, HairMax, or any low-level laser therapy), or scalp laser device, apply +3 to +5 on top of the stage baseline — clinical microneedling increases minoxidil follicular absorption up to 3× and independently stimulates dermal papilla growth signals; LLLT increases scalp circulation and prolongs anagen phase with a comparable evidence base. Apply only if minoxidil or another topical is also listed in "Current routine"; without a topical the amplification benefit is reduced and the adjustment is +1 to +2 only.
  Oral minoxidil upward adjustment: when "Current routine" explicitly lists oral minoxidil — recognizable as "oral minoxidil", "minoxidil tablet", "minoxidil pill", "low-dose oral minoxidil", "LDOM", "Loniten", or a dosage expressed in mg (e.g. "minoxidil 2.5mg", "0.625mg minoxidil") — apply +3 to +5 on top of the stage baseline, ABOVE the topical minoxidil baseline already captured by the stage range. Oral minoxidil bypasses the scalp absorption barrier entirely, delivering minoxidil systemically at a higher and more consistent follicular concentration than topical application; randomized head-to-head trials show oral minoxidil 2.5mg achieves greater density improvement than topical 5% at 24 weeks. Use the lower end (+3) when the oral dose is ≤1.25mg/day (low-dose regimen) and the higher end (+4 to +5) when the dose is 2.5mg/day or above, or when the dose is unspecified but the user confirms they are on a prescription oral tablet. Do NOT apply this adjustment when the routine lists only topical minoxidil (solution, foam, or spray) without an explicit oral/tablet/mg qualifier.
  ...
  Postpartum TE upward adjustment: when 'Onset' or 'Concerns' include any postpartum or post-pregnancy indicator AND stage is 'diffuse' or 'n/a (female)', apply +8 to +12 on top of the stage baseline — postpartum TE is the highest-reversibility TE variant; follicles are completely intact and recovery is essentially guaranteed once hormonal equilibrium restores within 6–12 months post-delivery. Use the higher end (+10 to +12) when delivery was within the past 12 months and shedding is clearly diffuse with no focal AGA signals. Do not apply when focal AGA-pattern loss (temple recession or crown bald patch) is clearly visible alongside the diffuse shedding — that warrants normal potential scoring. Treatment-onset TE takes priority if both apply.
  ...
  Non-AGA condition calibration for potential: when the photoNote flags a non-androgenetic condition, apply the following adjustments ON TOP OF the stage-based range above, because the recovery ceiling is fundamentally different from AGA:
  • Scarring alopecias (FFA, LPP, CCCA): apply −10 to −20 downward adjustment and cap potential at 45 maximum. Scarring alopecias destroy follicles permanently through perifollicular inflammation; affected zones cannot regrow once scarred. The realistic treatment goal is slowing progression, not recovery. The lower end (−18 to −20, cap 35) applies when the flagged zone appears extensive or the user is at a moderate-to-advanced AGA stage simultaneously; the higher end (−10 to −12, cap 45) applies when the flagged zone appears early or localized. Do not apply the scarring adjustment to unaffected AGA zones (e.g. if only the frontal hairline shows FFA and the crown is fully intact AGA-type thinning, apply the cap to frontal potential but do not reduce the crown assessment).
  • Alopecia areata (AA): apply +5 to +15 upward adjustment, with a ceiling of 88. AA follicles are NOT miniaturized or destroyed — they are in a reversible autoimmune block (catagen/telogen arrest) that preserves the follicle's regenerative capacity entirely. Recovery with appropriate immunotherapy (intralesional steroids, JAK inhibitors) is often complete in patchy AA. Use the lower end (+5 to +8) for extensive or totalis/universalis AA where spontaneous remission is less predictable; use the higher end (+10 to +15) for early patchy AA affecting limited zones where recovery probability is high. Do not confuse AA with AGA — the upward adjustment only applies when photoNote explicitly flags an AA pattern.
  • Traction alopecia (TA): apply +8 to +15 upward adjustment when TA is flagged and the loss appears early (no obvious permanent scarring at the hairline margin). TA is mechanically caused and highly reversible in the early and intermediate stages — removing the hairstyle trigger allows follicle recovery; recovery in early TA approaches that of TE. Use the lower end (+8 to +10) when the TA recession appears deeper or longer-standing; use the higher end (+12 to +15) when the recession appears recent and the hairline margin looks otherwise healthy. If there are signs of secondary fibrosis or scarring at the TA margin (an abnormally smooth, fibrotic hairline edge), apply no upward adjustment — scarring-stage TA has irreversible zones.
  • DUPA (Diffuse Unpatterned Alopecia): apply −5 to −10 downward adjustment. DUPA extends DHT sensitivity to the occipital and lateral fringe zones normally considered safe donor regions. While medical treatment (finasteride, minoxidil) can slow progression, DUPA patients have a poorer long-term density-recovery ceiling than standard AGA because even partially-responsive donor-zone hairs continue to miniaturize, and surgical restoration is typically contraindicated. Use the lower end (−5 to −7) when only mild fringe involvement is visible; use the higher end (−8 to −10) when fringe thinning is pronounced.
  • Seborrheic dermatitis / scalp psoriasis: no separate potential adjustment — SD and psoriasis are co-treatable alongside AGA. Score potential normally by AGA stage; the inflammatory condition is factored into the health score (via the −5 to −15 deduction above), not into the forward-looking potential score.
...
Use a balanced visual baseline: score what is actually visible in the photo and user context. Do not artificially lower scores for healthy-looking or stable-looking areas, and do not push users into a low range just for motivation. Typical mild early thinning: 62-78 overall; clearly healthy cases: 78-90; significant recession or density loss: 35-62; 91+ is rare. If photoQuality is 'poor', use a conservative uncertainty range around 62-76 and reflect uncertainty in the verdict. Never refuse — produce a best-effort estimate. Insights array MUST contain exactly 3 entries.`;
      ...
      const scanPromise = (async () => {
        ...
        // Computed once here and used throughout the server-side pipeline (urgency, coach questions, weeklyFocus).
        {
          ...
          // Detected from profile concerns/timeline keywords; not expected in photoNote (scan prompt focuses on AGA).
          // Only applies when postpartum_te is not already flagged (postpartum takes clinical priority).
          if (!_conds.includes('postpill_te') && !_conds.includes('postpartum_te')) {
            const _concernsL = (profile.concern || []).map(c => String(c).toLowerCase()).join(' ');
            const _timelineL = (profile.timeline || '').toLowerCase();
            const _combinedL = _concernsL + ' ' + _timelineL;
            const _isPostPill = /stopped.{0,15}pill|off.{0,10}pill|came off.{0,10}pill|stopped.{0,15}birth control|discontinued.{0,15}contraceptive|stopped.{0,15}ocp|stopped.{0,15}hormonal birth|quit.{0,10}pill|stopped.{0,15}the pill/i.test(_combinedL);
            if (_isPostPill) _conds.push('postpill_te');
          }
        ...
        if (!data.insights.some((ins) => ins.metric === _weakLabel)) {
          ...
          const _weakFallback = {
            ...
            Health: _isAdvancedForFallback
              ? { title: 'Protect scalp for fringe survival', body: `At ${data.stage} with scalp health at ${data.health}/100, a gentle sulfate-free shampoo and DHT-blocking shampoo 3× weekly protect the remaining follicles from inflammation — scalp environment directly determines how well topicals absorb into the fringe.`,                                          metric: 'Health'   }
              : data.stage === 'n/a (female)'
                ? { title: 'Investigate scalp health root cause', body: `For female-pattern thinning, scalp health at ${data.health}/100 often reflects hormonal or nutritional drivers — ask your doctor about a ferritin and hormone panel; low ferritin alone is a leading correctable cause of follicle miniaturization.`,                                                 metric: 'Health'   }
                : data.stage === 'diffuse'
                  ? { title: 'Rule out a reversible cause',   body: `Diffuse thinning with scalp health at ${data.health}/100 often has a correctable root cause — a blood panel checking ferritin, thyroid, and vitamin D is the highest-ROI step before adding products; treating the cause improves follicle health faster than topicals alone.`,                           metric: 'Health'   }
                  : { title: 'Optimize scalp environment',    body: `Switch to a gentle sulfate-free shampoo and add biotin and zinc — scalp health at ${data.health}/100 directly affects how well topicals absorb and follicles respond.`,                                                                                                                                   metric: 'Health'   },
            Potential: _isAdvancedForFallback
              ...
              : data.stage === 'n/a (female)'
                ? { title: 'Unlock hormonal treatment response', body: `Female-pattern potential at ${data.potential}/100 rises fastest when the hormonal root cause is identified — ask your doctor about a hormone and ferritin panel alongside starting topical minoxidil to the central part now while the response window is open.`,                                        metric: 'Potential'}
              : data.stage === 'diffuse'
                ? { title: 'Find the cause, then stack',    body: `Diffuse thinning potential at ${data.potential}/100 is highest when the underlying cause (ferritin, thyroid, or hormonal) is corrected first — get a blood panel this month, then add minoxidil and scalp massage once the root driver is addressed.`,                                                      metric: 'Potential'}
        ...
        const _hasFinasteride = _routineItems.some((r) => r.includes('finasteride') || r.includes('propecia') || r.includes('dutasteride') || r.includes('avodart') || r.includes('proscar') || r.includes('finpecia') || r.includes('finalo') || r.includes('finast') || r.includes('fincar') || r.includes('finax') || r.includes('aindeem') || r.includes('dutas') || r.includes('duprost') || r.includes('duodart') || r.includes('combodart') || r.includes('spironolactone') || r.includes('spiro') || r.includes('aldactone') || r.includes('bicalutamide') || r.includes('casodex') || r.includes('flutamide') || r.includes('cyproterone') || r.includes('androcur') || r.includes('clascoterone') || r.includes('winlevi') || r.includes('morr-f') || r.includes('morr f') || r.includes('tugain-f') || r.includes('tugain f'));
        // Oral minoxidil is an Rx-only vasodilator tablet (Loniten brand, low-dose off-label use for
        // hair loss). Unlike topical minoxidil it is prescription-only and has systemic effects.
        ...
        const _hasOralMinoxidil = _routineItems.some((r) => r.includes('loniten') || r === 'ldom' || r.includes('ldom ') || r.includes(' ldom') || (r.includes('oral') && (r.includes('minoxidil') || r.includes('minox'))) || (r.includes('minoxidil') && (r.includes('tablet') || r.includes('pill') || r.includes('low dose') || r.includes('low-dose') || r.includes('systemic'))) || (r.includes('minoxidil') && /\d+\.?\d*\s*mg/.test(r)));
        ...
        (() => {
          ...
          if (_dc.some(c => c === 'ffa' || c === 'lpp' || c === 'ccca')) {
            ...
            // Post-pill TE: reassure + direct toward the one modifiable lever (ferritin/nutrition).
        ...
        data.protocolCoverage = {
          topical:       _hasMinoxidil,       // minoxidil / rogaine (topical and oral brands; see oralMinoxidil)
          oralMinoxidil: _hasOralMinoxidil,   // loniten / oral minoxidil tablet — Rx prescription vasodilator
        ...
        if (_pqStageChange.stageDirection === 'progressed') {
          ...
          const _pqProgressionQ = _pqRx
            ...
            : _pqOnAnyTreatment
              ? stage === 'n/a (female)'
                ? `My scan progressed from ${previousStage} to female-pattern thinning despite OTC treatment — should I ask my doctor about spironolactone or other Rx options?`
        ...
        // postpill_te          → 42 days (OCP withdrawal TE; recovery over 6-12 months; 6-week check appropriate)
        ...
        if (data.detectedConditions.includes('treatment_induced_te') && data.checkInIntervalDays > 28) {
          ...
          data.checkInIntervalDays = 42;
        } else if (data.detectedConditions.includes('postpill_te') && data.checkInIntervalDays > 42) {
          data.checkInIntervalDays = 42;
        }
        ...
        } else if (data.detectedConditions.includes('postpill_te')) {
        ...
        // (India/South-Asia/UK brand names for finasteride) and dutas|duprost|duodart|combodart
  ...
  if (req.method === 'POST' && reqPath === '/api/coach') {
    try {
      ...
      const coachProtocolCoverage = (() => {
        ...
        if (ctx.routine.length > 0) {
          ...
          return {
            ...
            oralMinoxidil: r.some((s) => s.includes('loniten') || s === 'ldom' || s.includes('ldom ') || s.includes(' ldom') || (s.includes('oral') && (s.includes('minoxidil') || s.includes('minox'))) || (s.includes('minoxidil') && (s.includes('tablet') || s.includes('pill') || s.includes('low dose') || s.includes('low-dose') || s.includes('systemic'))) || (s.includes('minoxidil') && /\d+\.?\d*\s*mg/.test(s))),
            rx:          r.some((s) => s.includes('finasteride') || s.includes('propecia') || s.includes('dutasteride') || s.includes('avodart') || s.includes('proscar') || s.includes('finpecia') || s.includes('finalo') || s.includes('finast') || s.includes('fincar') || s.includes('finax') || s.includes('aindeem') || s.includes('dutas') || s.includes('duprost') || s.includes('duodart') || s.includes('combodart') || s.includes('spironolactone') || s.includes('spiro') || s.includes('aldactone') || s.includes('bicalutamide') || s.includes('casodex') || s.includes('flutamide') || s.includes('cyproterone') || s.includes('androcur') || s.includes('clascoterone') || s.includes('winlevi') || s.includes('morr-f') || s.includes('morr f') || s.includes('tugain-f') || s.includes('tugain f')),
            dhtShampoo:  r.some((s) => s.includes('dht') || s.includes('ketoconazole') || s.includes('nizoral') || s.includes('keto shampoo') || s.includes('caffeine shampoo') || s.includes('regenepure') || s.includes('alpecin') || s.includes('plantur') || s.includes('foligain') || s.includes('lipogaine') || s.includes('revita') || s.includes('pura d') || s.includes('shapiro md') || s.includes('rosemary oil') || s.includes('mielle') || s.includes('maple holistics') || s.includes('nioxin') || s.includes('keranique') || s.includes('ultrax') || s.includes('phytocyane') || s.includes('bioxsine') || s.includes('watermans') || s.includes('anaphase') || s.includes('vichy') || s.includes('dercos') || s.includes('klorane') || s.includes('rene furterer') || s.includes('triphasic') || s.includes('ducray') || s.includes('bioscalin') || s.includes('revivogen') || s.includes('pronexa') || s.includes('nizral') || s.includes('ketomac') || s.includes('ketomine')),
            ...
            transplant:    r.some((s) => s.includes('transplant') || s.includes('hair graft') || s.includes('graft') || s.includes('dhi') || (s.includes('fue') && !s.includes('fuel')) || (s.includes('fut') && !s.includes('future') && !s.includes('futile') && !s.includes('futuristic'))) || !!(pc?.transplant),
          };
        }
        return pc;
      })();
      ...
      const protocolStatusLine = (() => {
        ...
        if (pc.topical) {
          ...
          const _TOPICAL_LABELS = [
            ...
            ['finalo-f',         'Finalo-F (topical minoxidil 5% + finasteride 0.1% combo — apply to scalp twice daily)'],
            ['finalo f',         'Finalo-F (topical minoxidil 5% + finasteride 0.1% combo — apply to scalp twice daily)'],
            ['morr-f',           'Morr-F (topical minoxidil 5% + finasteride 0.1% combo — apply to scalp twice daily)'],
            ['morr f',           'Morr-F (topical minoxidil 5% + finasteride 0.1% combo — apply to scalp twice daily)'],
            ['morr',             'Morr (topical minoxidil 5% — apply to scalp twice daily)'],
            ['tugain-f',         'Tugain-F (topical minoxidil 5% + finasteride 0.1% combo — apply to scalp twice daily)'],
            ['tugain f',         'Tugain-F (topical minoxidil 5% + finasteride 0.1% combo — apply to scalp twice daily)'],
        ...
        if (pc.rx) {
          ...
          const _RX_LABELS = [
            ['finalo-f', 'topical finasteride 0.1% (via Finalo-F combo — applied to scalp, lower systemic absorption than oral finasteride 1mg; does not replace oral finasteride for systemic DHT suppression)'],
            ['finalo f', 'topical finasteride 0.1% (via Finalo-F combo — applied to scalp, lower systemic absorption than oral finasteride 1mg; does not replace oral finasteride for systemic DHT suppression)'],
            ['morr-f',   'topical finasteride 0.1% (via Morr-F combo — applied to scalp, lower systemic absorption than oral finasteride 1mg; does not replace oral finasteride for systemic DHT suppression)'],
            ['morr f',   'topical finasteride 0.1% (via Morr-F combo — applied to scalp, lower systemic absorption than oral finasteride 1mg; does not replace oral finasteride for systemic DHT suppression)'],
            ['tugain-f', 'topical finasteride 0.1% (via Tugain-F combo — applied to scalp, lower systemic absorption than oral finasteride 1mg; does not replace oral finasteride for systemic DHT suppression)'],
            ['tugain f', 'topical finasteride 0.1% (via Tugain-F combo — applied to scalp, lower systemic absorption than oral finasteride 1mg; does not replace oral finasteride for systemic DHT suppression)'],
            ...
            ['dutas', 'dutasteride'],       ['duprost', 'dutasteride'],   ['duodart', 'dutasteride'], ['combodart', 'dutasteride'],
          ...
          // Dosage-pattern fallback: "2.5mg minoxidil" or "minoxidil 5mg" won't match any keyword
          // in _RX_LABELS above (the keywords are brand/generic names, not dose patterns). Catch the
          // pattern here so the protocolStatusLine names the drug rather than silently omitting it.
          // The guard prevents duplicate entry if _OM_LABELS already matched "oral minoxidil" above.
          if (!_rxFound.some((l) => l.startsWith('oral minoxidil'))) {
            for (const r of _rl) {
              if (r.includes('minoxidil') && /\d+\.?\d*\s*mg/.test(r)) {
                _rxFound.push('oral minoxidil (Rx prescription tablet — NOT an antiandrogen)');
                break;
              }
            }
          }
        ...
        if (pc.oralMinoxidil) {
          ...
          // Brand/product lookup parallel to _RX_LABELS above — names the specific oral minoxidil
          // product so the coach can reference "Loniten" or "oral minoxidil tablet" accurately.
          const _OM_LABELS = [
            ['loniten', 'oral minoxidil (Loniten — Rx vasodilator tablet, NOT an antiandrogen)'],
            ['oral minoxidil', 'oral minoxidil (Rx prescription tablet — NOT an antiandrogen)'],
          ];
          let _omBrandLabel = null;
          for (const [kw, label] of _OM_LABELS) {
            if (_omRl.some((r) => r.includes(kw))) { _omBrandLabel = label; break; }
          }
          let _omDose = null;
          ...
          // Dose is the most specific identifier; fall back to brand label; then generic label.
          active.push(_omDose
      ...
      const systemPrompt = [
        ...
        'Seasonal shedding calibration: a well-documented secondary TE trigger is seasonal photoperiod — increased shedding is most common in late summer to early autumn (roughly August–November in the Northern Hemisphere, February–May in the Southern Hemisphere), peaking approximately 90–120 days after the summer solstice as the hair cycle responds to changing day-length cues. Seasonal TE is bilateral, diffuse in pattern, self-limiting (typically resolves within 2–3 months), and does NOT indicate AGA progression or treatment failure. Use today\'s date (provided below) to determine whether the user is in the seasonal shedding window. When: (1) today\'s date falls in the seasonal risk window, AND (2) the user reports new or increased shedding without a clear AGA progression pattern in their last scan, AND (3) their treatment routine has been stable for 6+ months (no new medication starts in that period) — mention seasonal TE as a plausible contributing factor alongside any AGA context. Reassure the user it is temporary and does not require a treatment change; recommend monitoring with the next scan in 6–8 weeks rather than reacting immediately. Do NOT apply this when a treatment-onset TE context exists (new minoxidil, finasteride, or dutasteride start within the past 6 months) — treatment-onset TE is a more specific explanation that takes priority. Do NOT apply when the user\'s last scan shows clear AGA stage progression (stageChanged: true, direction: progressed) — that warrants an action recommendation, not seasonal reassurance.',
        'Postpartum TE handling: when the scan clinical note or the user\'s own messages indicate postpartum or post-pregnancy hair shedding ("postpartum", "after baby", "after delivery", "after giving birth", "months after baby", "postnatal", or similar context), apply the following calibration in every response: (1) REASSURE — postpartum telogen effluvium (TE) is the most common and most reversible form of hair shedding. It affects 40–50% of new mothers, is driven by the sudden drop in estrogen after delivery (not by follicle damage), and is essentially guaranteed to fully resolve. Shedding typically peaks 3–5 months after delivery and hair returns to its pre-pregnancy state within 6–12 months in the vast majority of cases — follicles are completely healthy and intact. This is the single most important thing to communicate. (2) SAFETY — CRITICAL breastfeeding drug warning: finasteride and dutasteride are absolutely contraindicated during pregnancy and breastfeeding (FDA Category X). Do NOT suggest or imply that a breastfeeding or recently postpartum user start finasteride or dutasteride — if these are already in their routine, advise them to consult their OB/GYN or doctor about safety while breastfeeding, and do NOT reinforce continued use. Oral minoxidil (systemic/tablet form) is also contraindicated during breastfeeding due to potential cardiovascular effects on the infant — never recommend it for a postpartum user who may be breastfeeding, and flag this risk if it comes up. Topical minoxidil evidence during breastfeeding is limited; systemic absorption is low but not zero — when asked, recommend consulting their OB/GYN or dermatologist before using topical minoxidil while breastfeeding, rather than giving a definitive safe/unsafe answer. (3) WHAT DOES HELP — the highest-ROI actions for postpartum TE are: (a) address nutritional deficiencies — ferritin/iron levels often drop post-delivery and low ferritin (below 40–70 ng/mL) independently extends TE shedding; recommend getting a blood panel (ferritin, iron, B12, vitamin D, zinc); (b) safe supplementation: iron (if deficient), biotin (safe during breastfeeding), zinc, vitamin D — these are all safe while breastfeeding; (c) patience — no treatment will significantly accelerate the natural 6–12 month recovery timeline, but consistent scalp health habits (DHT-blocking shampoo, scalp massage) maintain the environment for optimal regrowth; (d) topical minoxidil is the most evidence-supported topical option if the user is not breastfeeding or has cleared it with their doctor. (4) AVOID PANIC — do not frame postpartum TE as "hair loss" in the AGA sense; the follicles are not damaged, the shedding is hormonal timing, and recovery is the expected outcome. Reframe any AGA-stage-based alarm the scores might suggest — explain that current scores reflect the TE state, not a baseline prognosis. (5) WHEN CO-EXISTING AGA IS PRESENT — if the user has both postpartum TE AND a prior history of AGA, reassure about the TE component but acknowledge that any underlying AGA pattern (temple recession, crown thinning) will need separate long-term management once postpartum recovery completes and breastfeeding ends.',
        'Treatment-induced TE handling: when the scan context shows a treatment_induced_te condition flag, OR when the user\'s routine includes minoxidil, finasteride, dutasteride, spironolactone, bicalutamide, flutamide, cyproterone, or androcur AND their onset was within the past 6 months AND they are asking about increased shedding — apply the following calibration in every response: (1) REASSURE — treatment-onset telogen effluvium is a well-documented, expected pharmacological response, NOT a sign of treatment failure or worsening hair loss. The follicles are healthy and resetting; shedding occurs because the drug is pushing more follicles simultaneously into a growth phase (anagen), which first requires a brief resting phase (telogen) before the new growth cycle begins. This is a positive signal that the treatment is pharmacologically active. (2) TREATMENT-SPECIFIC TIMEFRAMES — minoxidil-onset TE: shedding typically begins within 2–6 weeks of starting and resolves within 1–3 months of consistent use; visible regrowth and stabilization become apparent at months 4–6. Finasteride-onset TE: shedding may begin in the first few weeks and typically resolves by months 3–4, occasionally persisting until month 6; the initial DHT suppression disrupts the existing hair cycle before stabilizing it. Dutasteride-onset TE: the initial shedding response can be slightly more pronounced than finasteride because dutasteride blocks both type 1 and type 2 5-alpha reductase (deeper DHT suppression); the shedding phase may persist until month 5–7 before the hair cycle fully restabilizes — this is normal and the outcome is typically superior stabilization to finasteride. Spironolactone-onset TE: spironolactone is a mineralocorticoid/androgen receptor blocker prescribed primarily for female-pattern hair loss; some users experience diffuse shedding in the first 1–4 months as the antiandrogen effect modulates androgen-receptor signaling in the hair follicle cycle. This is self-limiting and typically resolves by month 3–5 — the mechanism differs from 5-alpha reductase inhibitors (spiro blocks androgen receptors rather than suppressing DHT production) but the reassurance is the same: stay consistent, follicles are healthy and intact. Bicalutamide/flutamide/cyproterone-onset TE: bicalutamide (Casodex), flutamide (Eulexin), and cyproterone acetate (Androcur) are full androgen receptor antagonists used for female-pattern hair loss (especially when other antiandrogens are not tolerated) and in trans-feminine hormone therapy; some users experience a transient diffuse shedding adjustment in the first 1–4 months as androgen receptor blockade modulates follicle cycle timing. The mechanism is essentially the same as spironolactone-onset TE (receptor blockade rather than DHT production suppression), so the reassurance and action are identical: the shedding is expected, follicles are intact, and stopping is the single biggest mistake. Typical resolution: flutamide and cyproterone — month 2–4; bicalutamide — month 3–5 (slightly longer due to bicalutamide\'s longer receptor-binding half-life). (3) THE CRITICAL MISTAKE TO PREVENT — stopping treatment at the first sign of shedding is the single most common treatment error in hair loss management. Users who quit during the initial TE phase never reach the stabilization and regrowth phase that follows. Reinforce strongly: stay consistent. The shedding is evidence the drug is working, not evidence of failure. (4) WHEN TO REASSESS — recommend continuing the current treatment and scheduling a new scan at 4–6 months to confirm stabilization. If the user is MORE than 6 months into treatment and shedding is still accelerating (not plateauing or improving), that warrants a dermatologist or GP consultation to rule out a co-existing cause (nutritional TE, another trigger, or underlying condition making AGA worse). Do NOT apply treatment-induced TE reassurance beyond the 6-month window without clarifying that prolonged shedding needs medical evaluation.',
        'Post-pill TE handling: when the scan context shows a postpill_te condition flag, OR when the user\'s messages indicate they recently stopped oral contraceptives / birth control pills ("stopped the pill", "off birth control", "came off the pill", "stopped my OCP", "stopped hormonal birth control", or similar) AND they are asking about increased shedding — apply the following calibration in every response: (1) REASSURE — post-pill telogen effluvium is one of the most well-documented and reversible forms of hair shedding in women. It is caused by the sudden drop in estrogen when stopping OCP, which disrupts the hair cycle timing; follicles are completely healthy and intact. Shedding typically begins 2–4 months after stopping the pill and resolves within 6–12 months in the vast majority of cases as the body re-establishes its natural hormonal rhythm. This is the single most important message: temporary, hormonal, expected, and fully reversible. (2) DRUG SAFETY — unlike postpartum TE, there are NO breastfeeding safety concerns specific to post-pill TE. Topical minoxidil (Rogaine) is safe and evidence-supported for women who are not pregnant or breastfeeding, and can be freely recommended as an option while waiting for natural recovery. Finasteride and dutasteride remain contraindicated for women of childbearing potential in most jurisdictions and should only be discussed with reference to a prescribing physician. (3) WHAT DOES HELP — the highest-ROI actions for post-pill TE are: (a) check ferritin and iron levels — low ferritin (below 40–70 ng/mL) independently extends TE shedding regardless of the trigger; this is the single most actionable lab test to order; (b) safe supplementation: iron (if deficient), biotin, zinc, vitamin D; (c) topical minoxidil — if the user is not pregnant or breastfeeding, topical minoxidil can modestly support regrowth and is the most evidence-backed topical option during the recovery window; (d) patience — the natural 6–12 month recovery timeline cannot be dramatically shortened, but consistent scalp health habits (DHT-blocking shampoo, scalp massage) maintain the follicle environment; (e) if shedding is unusually severe or accompanied by other hormonal symptoms (irregular cycles, weight changes, fatigue), a blood panel including thyroid, ferritin, and hormones (estradiol, testosterone, SHBG) is worth requesting from their GP. (4) DISTINGUISH FROM POSTPARTUM TE — post-pill TE is hormonally similar to postpartum TE (both are estrogen-withdrawal driven) but has no breastfeeding or pregnancy safety constraints. Do not apply breastfeeding drug warnings (from the postpartum TE handling instructions) to post-pill TE users unless the user specifically indicates they are also breastfeeding. (5) AVOID PANIC — the follicles are not damaged. Do not frame post-pill TE as AGA or permanent loss. If the scan\'s Norwood stage or density scores look alarming, explain that the scores reflect the TE state at time of scan and are expected to improve as the hormonal cycle normalizes.',
        'Non-AGA condition handling: when the scan clinical note (see "Scan clinical note" below) flags a condition that is NOT standard androgenetic alopecia — specifically Frontal Fibrosing Alopecia (FFA), Lichen Planopilaris (LPP), alopecia areata, Diffuse Unpatterned Alopecia (DUPA), traction alopecia, Central Centrifugal Cicatricial Alopecia (CCCA), or seborrheic dermatitis / scalp psoriasis — ALWAYS address it directly before or alongside standard AGA-protocol recommendations. These conditions have distinct first-line treatments: FFA requires anti-inflammatory management (not finasteride/minoxidil alone) and URGENT specialist referral; LPP (Lichen Planopilaris) is a primary lymphocytic scarring alopecia causing irreversible follicle destruction through perifollicular inflammation — it requires anti-inflammatory treatment (hydroxychloroquine, cyclosporine, or intralesional/topical corticosteroids) and URGENT specialist referral because scarring is irreversible and standard AGA topicals/DHT blockers are not adequate as primary therapy; alopecia areata is autoimmune and needs intralesional steroids or JAK inhibitor evaluation (not AGA topicals); DUPA makes patients poor transplant candidates and needs dermoscopic donor assessment; traction alopecia resolves primarily by removing the hairstyle trigger; CCCA is a primary scarring alopecia most common in Black women that begins at the crown vertex — it requires anti-inflammatory treatment (intralesional/topical corticosteroids, doxycycline, hydroxychloroquine, or 5-alpha reductase inhibitors as adjuncts) and URGENT specialist referral because scarring is irreversible and early diagnosis significantly slows progression; seborrheic dermatitis (SD) and scalp psoriasis require antifungal or anti-inflammatory scalp treatment — for SD specifically, ketoconazole 2% shampoo (Nizoral or generic ketoconazole) 2–3× weekly is the evidence-based OTC first-line treatment; it also has mild anti-androgenic effects on the scalp, making it a valuable add-on to an AGA protocol. If the scan note flags FFA, LPP, alopecia areata, DUPA, traction alopecia, or CCCA: (1) lead with a brief acknowledgment that the scan detected a pattern that may not be standard AGA, (2) strongly recommend a dermatologist or trichologist consultation before starting or continuing AGA-specific treatments, (3) briefly explain why the standard AGA protocol may be insufficient for this pattern. Do NOT suggest AGA treatments (finasteride, DHT blockers) as the primary or sole recommendation when FFA, LPP, AA, DUPA, traction alopecia, or CCCA is flagged. If the scan note flags seborrheic dermatitis or scalp psoriasis: (1) acknowledge the inflammatory scalp condition, (2) recommend ketoconazole 2% shampoo as the OTC first-line for SD (it is freely recommendable, not Rx), or a dermatologist visit for psoriasis; (3) note that treating the scalp inflammation alongside AGA gives better hair health outcomes than AGA treatment alone — these conditions are co-treatable, so continue recommending their AGA protocol alongside the ketoconazole shampoo.',
        '',
