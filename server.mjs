...
Use a balanced visual baseline: score what is actually visible in the photo and user context. Do not artificially lower scores for healthy-looking or stable-looking areas, and do not push users into a low range just for motivation. Typical mild early thinning: 62-78 overall; clearly healthy cases: 78-90; significant recession or density loss: 35-62; 91+ is rare. If photoQuality is 'poor', use a conservative uncertainty range around 62-76 and reflect uncertainty in the verdict. Never refuse — produce a best-effort estimate. Insights array MUST contain exactly 3 entries.`;
      ...
      const scanPromise = (async () => {
        ...
        const _hasMassage    = _routineItems.some((r) => r.includes('massage') || r.includes('dermaroller') || r.includes('derma roller') || r.includes('dermapen') || r.includes('microneedl') || r.includes('micro-needl') || r.includes('lllt') || r.includes('laser cap') || r.includes('laser comb') || r.includes('capillus') || r.includes('hairmax') || r.includes('irestore') || r.includes('igrow') || r.includes('theradome') || r.includes('kiierr') || r.includes('illumiflow'));
      ...
      const protocolStatusLine = (() => {
        ...
        if (!pc && ctx.routine.length > 0) {
          ...
          pc = {
            ...
            mechanical:    r.some((s) => s.includes('massage') || s.includes('dermaroller') || s.includes('derma roller') || s.includes('dermapen') || s.includes('microneedl') || s.includes('micro-needl') || s.includes('lllt') || s.includes('laser cap') || s.includes('laser comb') || s.includes('capillus') || s.includes('hairmax') || s.includes('irestore') || s.includes('igrow') || s.includes('theradome') || s.includes('kiierr') || s.includes('illumiflow')),
